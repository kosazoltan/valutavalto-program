...
begin
  ...
  if _kertDatum>date then
    begin
      Showmessage('A KÉRT NAP MÉG A JÖVŐBEN VAN');
      exit;
    end;
...
begin
  ...
  if _tipus='F' then IranyNevPanel.caption := 'Átvevő pénztár'
...
begin
  ...
  if (_ugyfeltipus='N') or (_ugyfeltipus='K') then
    begin
      ...
      if _kozszereplo<1 then KOZszereploedit.Text := 'NEM KIEMELT KÖZSZEREPLŐ'
      else KozSzereploEdit.text := 'KIEMELT KÖZSZEREPLŐ';
      KozSzereploedit.repaint;
...
begin
  ...
  else BlokkfejPanel.caption := 'Blokkfejek (szűrve)';
...
begin
  ...
  if _tkozszer=0 then TKozszerPanel.Caption := 'NEM KIEMELT KOZSZEREPLŐ'
  else TKozszerPanel.Caption := 'KIEMELT KÖZSZERELŐ ('+inttostr(_tkozszer)+')';
