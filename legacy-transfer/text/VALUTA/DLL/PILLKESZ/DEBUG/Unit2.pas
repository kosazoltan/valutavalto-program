...
uses
  ...
  Dialogs, Unit1, ExtCtrls, StdCtrls, wininet, IBDatabase, DB, IBQuery,
...
var
  ...
  _mResult,_gepfunkcio,_lastirodasorszam,_ss,_sIrodaDarab: integer;
...
// =============================================================================
  ...
  Feladata: A c:\valuta\irodak\irodak.dat nevü file-ból kell kinyerni az
              ...
              FTP-szerverről az \IRODAK nevü könyvtár IRODAK.DAT nevü
              file-jával.

              Ha nem tudja frissiteni, akkor a meglévő adatokat használja fel.
...
begin
  ...
  serverDbase.close;
  Serverdbase.DatabaseName := _host + ':C:\RECEPTOR\DATABASE\RECEPTOR.FDB';
  ...
  Serverdbase.DatabaseName := _host + ':c:\cartcash\database\artcash.fdb';
  Serverdbase.Connected := true;
...
begin
  ...
  if _hFTP=nil then
    begin
      Showmessage('A szerver nem érhető el !');
      InternetCloseHandle(_hNet);
    end;
end;
...
begin
  ...
  _pcs2 := 'SELECT * FROM ARFOLYAM WHERE VALUTANEM='+chr(39)+'USD'+CHR(39);
