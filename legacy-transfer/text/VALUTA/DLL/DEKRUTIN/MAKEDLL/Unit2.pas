...
begin
  ...
  if _kertkezdonap>_aktdatums then
    begin
      ShowMessage('A KÉRT DÁTUM A JÖVŐBEN LESZ !');
      ModalResult := 2;
      exit;
    end;
...
begin
  ...
  logirorutin(pchar(_mess));
  logirorutin(pchar('Kezdősorszam: '+inttostr(_kezdosorszam)));
  ...
  logirorutin(pchar('Korrigált végső sorszám='+inttostr(_vegsosorszam)));
...
begin
  ...
  VonalHuzas;
   ...
   // A kezdő sorszámmal indulnak az adatsorok:
  ...
  while _y<=_xsordb do
   ...
   // A dekádjelentésből kiolvasott vétel sorát irja fel (ha volt vétel)
  ...
  if _vetel>0 then
   ...
   // A dekádjelentésből kiolvasott eladás sorát irja fel (ha volt eladás);
...
begin
  // A BF farok havi gyüjtő adatait olvassa be a dekád időszaka alatt:
...
begin
  ...
  if _recno=0 then
    begin
      _kss := inputbox('SORSZÁMBEKÉRŐ','Kérem a dekád első sorszámát:','0');
