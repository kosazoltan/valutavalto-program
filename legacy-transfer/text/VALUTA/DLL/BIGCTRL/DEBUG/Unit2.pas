...
begin
  ...
  if _mResult=-1 then
    begin
      ...
      logirorutin(pchar('szerver nem érhető el. Kilépek'));
  ...
  if _tranztip=0 then
    begin
      logirorutin(pchar('Nincs semmi megszoritás - könyvelhető'));
      Konyveles;
      _Mresult := 1;
      Kilep.Enabled := True;
      exit;
...
begin
  ...
  Valutadbase.connected := true;
   ...
   logirorutin(pchar('Főbb adatok: '+ _mess));
...
begin
  ...
  // A Jogiszemélynév első 7 karaktere alapján keresem:
  ...
  if not _megvan then
    begin
      ...
      logirorutin(pchar('Nincs egyező adat - igy új jogi ügyfél ez'));
      exit;
    end;
...
begin
  ...
  // Ha belföldi és kiemelt közszereplő -> Tranztipus = 1
