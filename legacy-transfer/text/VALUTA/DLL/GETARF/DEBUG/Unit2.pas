...
begin
  ...
  // Eredmény-jelzők ektüntetése:

  ChangePanel.Visible   := False;
  NochangePanel.Visible := False;

  // Kiteszi az ablakot a képernyőre:
  ...
  GetGepParameters;  // gépfunkció és pénztárszám meghatározása
   ...
   // Kiolvassa a gépen levő NR*.dat-t vagy NR0000000.dat nevét

  _currate := GetCurrate;
  _currate := uppercase(trim(_currate));

  // Ha kettő a paraméter - akkor MNB letöltés
...
begin
  ...
  // Első feladat: az internet ellenőrzése, illetve bekapcsolása:

  Memotabla.Lines.Add('Internet ellenőrzése ...');
...
begin
  ...
  if _hFTP=nil then
    begin
      ShowMessage('A központi szerver nem érhető el !');
      InternetCloseHandle(_hNet);
    end;
end;
