...
begin
  ...
  _mess[1] := 'KEZELÉSI DÍJ FELEZÉSE ÜGYVEZETŐ VAGY FŐÉRTÉKTÁROS ENGEDÉLYÉVEL';
  _mess[2] := 'KEZELÉSI DÍJ FELEZÉSE AKCIÓ KERETÉBEN';
  _mess[3] := 'KÁRTYÁS KEZELÉSI DÍJ FELEZÉS';
  _mess[4] := 'KEZELÉSI DÍJ ELENGEDÉSE ÜGYVEZ VAGY FŐÉRTÉKTÁROS ENGEDÉLYÉVEL';
  _mess[5] := 'KÁRTYÁS KEZELÉSI DÍJ ELENGEDÉSE';
  _mess[6] := 'EGYEDI KEZELÉSI DIJ (Max: 3 %)';
  _mess[7] := 'SPECIÁLIS KEZELÉSI DÍJ';
...
begin
  ...
  if (_tag=6) and (_napikezdij>2) then
    begin
      ShowMessage('A napi egyedi kezdij lehetősége kimerült');
      Modalresult := -1;
      exit;
    end;
