...
type
  TKORLEVEL = class(TForm)
    ...
    procedure LEVELRACSKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
//  _remoteKorFdb: string = '185.43.207.99:c:\receptor\database\korltest.fdb';

  _remoteKorFdb: string = '185.43.207.99:c:\receptor\database\korlevel.fdb';
  _remoteProsFdb: string = '185.43.207.99:c:\receptor\database\ptarosok.fdb';
  ...
  _localPath,_remotefile,_aktidkod,_sofficepath,_mamas: string;
...
begin
  ...
  // Paramétertől függöen: 1= Letöltés, 2=Olvasás
...
begin
  ...
  // Beolvassa a szerver-adatbázisból a kért körlevél  mezőadatait:
...
// =============================================================================
{
    ...
    Saját gépről= _aktidkod   = VALUTA.FDB -> HARDWARE -> IDKOD
                  _lastloaded = KORLEVEL.FDB -> PARAMETERS -> LASTSORSZAM

    Szerverről  = _lastletter = (ip)KORLEVEL.FDB -> ADATOK -> LASTSORSZAM
...
begin
  ...
  with ServerQuery do
    begin
      ...
      Open;
      _lastletter := FieldByname('UTSORSZAM').asInteger;
      close;
    end;
  Serverdbase.close;
