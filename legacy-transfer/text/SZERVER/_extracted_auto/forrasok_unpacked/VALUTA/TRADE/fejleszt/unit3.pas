...
begin
  ...
  // Az időtartam lehet: HETI, HAVI, EVES

  _idotartam := Getidotartam(_pTag);

  // Kiszámolja a start napot és időt, az end napot és időt
  // és fizetendő összeget:
...
// Kiszámolja a matrica érvényességi időtartamát és a fizetendő összeget
...
// ITT JÓVÁHAGYJA A MATRICA-ADATLAPON SZEREPLŐ ADATOK HELYESSÉGÉT
...
// Az összes adat kijelzése után megerősiti, hogy kéri elözetes igénylést
...
begin
  ...
  if _reppieces=0 then
    begin
      ...
      //  Kijelzi a hibaüzenetet, majd TOVÁBB-gomb modalresult=2 -> főmenü
      HibaUzenet;
      Exit;
    end;

  // Itt megjött a válasz, lehet értékelni a benne lévő adatokat:
...
begin
  ...
  if _akcio = 'TEST' then
    begin
      ...
      // A visszaküldött fizetendő összeg kibontása és kijelzése
...
TestAAK válasz
        ...
        <DurationHUN>heti/10 napos (országos) </DurationHUN> időszak szövegesen magyarul
        <DurationEN>weekly/10 days (national)</DurationEN> időszak szövegesen angolul
...
LiveAAK küldés végpontról
    ...
    1)	17 hosszú kártyaszám küldése. Ilyenkor nincs Restriction kód ellenőrzés
    2)	17 hosszú kártyaszám és az azt követő 9 karakter küldés (összesen 26 karakter): Ebben az esetben ellenőrzésre kerül a Restriction kód is.
...
LiveAAK válasz
   ...
   A <Print2> tag-ben lévő információt figyelmen kívül kell hagyni, kivéve, ha EW tranzakció esetén a fizetési tranzakció számát is a bizonylatra szeretnénk nyomtatni.
...
e-matrica ellenőrző szelvény / e-vignette control slip
...
Vásárlás időpontja / Date of purchase: 	 éééé.hh.nn óó:pp:ss /yyyy.mm.dd hour:min:sec
...
--------------------------------------------------------------------------------
   ...
   Vevői példány adattartalma/Content of the Customer's copy


    e-matrica ellenőrző szelvény / e-vignette control slip
                 Vevői példány / Customer's copy
                     Nem adóügyi bizonylat!
                     Not taxation document!
...
Ár / Price: XXXXX  HUF
   ...
   Az adatok helyszínen történő módosítását a vásárlástól számított "x" percen
  belül lehet kezdeményezni./ Modification of the data on site can be initiated
           ...
           Kérjük a bizonylatot a lejárat után 2 évig megőrizni!
