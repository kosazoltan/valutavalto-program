...
var
  ...
  _napnev: array[1..7] of string = ('HÉTFŐ','KEDD','SZERDA','CSÜTÖRTÖK',
...
begin
  ...
  if _zarodstring>_megnyitottnap then
    begin
      ...
      ShowMessage('A megjelölt nap a jövőben lesz !');
      ModalResult := 2;
      Exit;
    end;
  ...
  (* A tárgyév és tárgyhónap alapján meghatározza a gyüjtő adatbázisok farkát *)
...
//  if _kellForgalom=1 then UgyfAndJogiPack;
  ...
  Memo1.Lines.add(' ...');
  Memo1.Lines.Add('Az esti csomag küldésre előkészítve');
...
begin
  ...
  if fileExists('c:\valuta\' +_localPath) then
    ...
    end else logirorutin(pchar('Nincs napi jelentés előkészitve'));
end;
...
begin
  ...
  Putbyte(4);  // 4 mező van a tételekben;
...
begin
   ...
   // A napi összesitőben van a mai napi TRADE forgalom:
   ...
   // A zárónapi rekord mezőit beolvassa és zárja a napiosszesito táblát:
     ...
     begin
       ...
       while not MatricaQuery.Eof do
         begin
           ...
           _irany := 45;  // minusz
           if _ftd<0 then  // forditott előjelek !!!!!
             begin
               _irany := 43;  // plusz +
               _ftd := abs(_ftd);
             end;
