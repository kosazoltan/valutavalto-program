...
begin
  ...
  // _dnemdarab : _DNEM ÉS ELSZARF tömbelem feltöltése ARFOLYAM.FDB-bol #EUA
  ...
  // Az idöstring beolvasása:

  _aktIdos           := GetIdos;

  // A föfelirat és havi-farok neghatározása:

  _datumtext := _zDatums + '-i zárás nyomtatása';
  ...
  // Az Aktlist.txt föcimének és dátumának beirása:
...
//  if _navOke<>2 then PenztarAllas;
  ...
  // Napi zárókészlet kiirása:

  Csik.Position := 30;
  NapZaroKeszletek;

  // A napi forgalom kiirása:
...
//  FoglaloKeszletNyomtatas;
  ...
  // A wzaryymm-bõl kiolvassa az adatokat:

  Csik.Position := 50;
  if (_kellwestern=1) or (_kelltesco=1) or (_kellmetro=1) then
    begin
      WuWaDataRead;
      // Az wunion és wafa textfileba irása:
      Wuwalista;
    end;
  Csik.Position := 60;
...
begin
  ...
  _tetFileNev := 'BT'+_farok;
   ...
   // A blokkfejgyüjtö megnyitása: szürve a zárás napjára
  ...
  _eKerekites := 0;
   ...
   // Végigmegy a mai napi blokkokon
...
KEZELSI KOLTSEG .......: 123,456,000 Ft
...
B-123456   atadas  -123,456,789 FT  1234
----------------------------------------
    HORVÁT KUNA 2012.12.12-I LISTAJA
----------------------------------------
...
123456789012345678901234567890123456789
   ...
   ----------------------------------
    Napi nyito összeg  : 1.234.567 Ft
