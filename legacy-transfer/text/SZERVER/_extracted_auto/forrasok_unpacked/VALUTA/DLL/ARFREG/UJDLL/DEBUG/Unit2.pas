...
begin
  ...
  //   _ratedatum[1.._datumdb] - a havi adatbázisban lévő dátumok nevei (string)
...
// A kert-datum és kert-ido -nek megfelelő adatok beolvasása és betöltése
//                   az 'y' - kezdetű tömbökbe:

begin
  // A 'SARFTABLA'-ból újra megnyitja kertdatum és kertidő szüréssel:
  ...
  // Az adatok betöltése a szürt adatbázisból az 'y' - kezdetű törmbökbe:
  _qq := 1;
  while not ArfolyamQuery.eof do
    begin
      with ArfolyamQuery do
...
begin
  // Főcim összeállitás: dátum és idő:
  ...
  // A valutadarabnyi cellak feltöltése a 'y'-nal kezdödő tömbök adataival:
