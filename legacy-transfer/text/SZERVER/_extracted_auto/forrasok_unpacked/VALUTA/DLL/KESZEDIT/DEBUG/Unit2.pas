...
type
  TForm2 = class(TForm)
   ...
   function Ftform(_ft: integer): string;
    ...
    function VOltmar(_vs: string): byte;
...
var
 ...
 _dnev: array[1..27] of string = ('AUSZTRAL DOLLAR','BOSNYAK MARKA','BOLGAR LEVA',
  ...
  _cimlista: string ='c:\valuta\cimlprnt.txt';
...
function kcimletrutin(_para: byte): integer; stdcall; external 'c:\valuta\bin\kcimlet.dll'
                                  name 'kcimletrutin';
...
begin
  ...
  logirorutin(pchar('Indul a mozgatandó valutár listája'));

  logirorutin(pchar('Törli a cimletpiszkozatot'));
    ...
    begin
      ...
      if length(_aktstring)<3 then
        ...
        begin
          ...
          _yy := voltmar(_aktstring);

          if (_xx=0) or (_yy=1) then    //  Nincs !!
            begin
              _dn[_pss].caption := '';
              _aktPanel := _dn[_pss];
              dn1click(_aktpanel);
...
begin
  ...
  if _bill=39 then
   ...
   // enter - jöhet a következő valuta:
...
begin
  ...
  if _aktdnem='JPY' then  _oszto := 1000;
  _aktertek := round(_aktbankjegy/_oszto*_aktrealarf);
...
begin
  logirorutin(pchar('End gombot nyomott - nincs tübb adatsor'));
  logirorutin(pchar('Összesen '+inttostr(_seldb)+' tételt talált'));
   _y := 1;
  _sumertek := 0;
  while _y<=_seldb do
    begin
      ...
      logirorutin(pchar(_aktdnem+'->'+inttostr(_aktertek)+'='+inttostr(_sumertek)));

      if _aktcimlet=0 then
        begin
          _pcs := 'INSERT INTO CIMLETPISZKOZAT (VALUTANEM,VALUTANEV,BANKJEGY,';
          _pcs := _pcs + 'ARFOLYAM,TORTRESZ,OSSZESFORINTERTEK)'+ _sorveg;
        ...
        begin
          ...
          _pcs := _pcs + ',OSSZESFORINTERTEK=' + inttostr(_aktertek)+_sorveg;
  ...
  logirorutin(pchar(inttostr(_seldb)+' db '+inttostr(_sumertek)+' Ft értékben'));
...
function TForm2.VOltmar(_vs: string): byte;
