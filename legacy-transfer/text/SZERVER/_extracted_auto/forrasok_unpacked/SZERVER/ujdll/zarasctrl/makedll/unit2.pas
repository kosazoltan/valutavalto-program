...
begin
  ...
  if _index>150 then acdbookParancs(_pcs);
...
begin
  AcDbookDbase.close;
  AcDbookDbase.DatabaseName := 'c:\cartcash\database\daybook.fdb';
...
begin
  ...
  while not Dbooktabla.Eof do
    begin
      _stat := Dbooktabla.FieldByName(_aktMezo).asString;
      if _stat<>'B' then
         begin
           Dbooktabla.edit;
  ...
  while not AcDbooktabla.Eof do
    begin
      _stat := AcDbooktabla.FieldByName(_aktMezo).asString;
      if _stat<>'B' then
         begin
           AcDbooktabla.edit;
