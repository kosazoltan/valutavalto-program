...
var
  ...
  _pTegla: array[1..27,1..10] of TPanel;
...
begin
  ...
  _jTop  := 97 + 23*(_startSor-1);
  _jHeight := 23*(_endSor-_startSor+1)+3;
  ...
  if shift<>[ssCtrl..ssLeft] then
   ...
   //   if _isMarker then exit;
      ...
      //    Fuggvenykijelzes;
          ...
          // A kezdő és végkoordináták azonosak:
          _startoszlop := _aktoszlop;
          _endoszlop   := _aktoszlop;
          _startsor    := _aktsor;
          _endsor      := _aktsor;

          // Aláteszi a piros jelölőpanelt:
...
begin
  ...
  // Ha itt ,ég nincs kijelölésről szó - függvénykijelzés
  if not _signing then
    begin
      FuggvenyKijelzes;
      exit;
...
(*
// =============================================================================
              procedure TMUNKAFORM.MMENUBETimer(Sender: TObject);
// =============================================================================

begin
  MMenube.Enabled := false;
  _mwidth := _mwidth + 44;
  _mHeight := _mheight + 27;
  MunkaMenupanel.Width := _mWidth;
  MunkamenuPanel.Height := _mHeight;
  if _mwidth<440 then
    begin
      Mmenube.Enabled := true;
      exit;
    end;
  Megsemgomb.cancel := True;
