...
{$R *.dfm}
...

 {
    ...
    - ha nem sikerül valamiért az Internetről, akkor megkérdezi, hogy
...
begin
  ...
  if _hftp=Nil then
    begin
       ListBox1.items.add('Megpróbálom a pécsi IP-címen !');
       _hFTP := InternetConnect(_hNet,Pchar(_pecsHost),_pecsPort,Pchar(_userId),
                           PChar(_ftpPassword),INTERNET_SERVICE_FTP,
                           INTERNET_FLAG_PASSIVE,
                           0);
    end;
  ...
  ListBox1.Items.Add(_remoteFile+' letöltése a szerverről ...');
...
(*
   Bedolgozza:
     ...
     - az alaplap árfolyamait: 27 valutanem, 9 oszlopának 4 propertyjét

     - az aktiv munkacsoport darabszámát;
     - az aktiv munkacsoportok megnevezését.

     - a munkacsoportok 27 valutanemre szóló 8 oszlopának 4 propertyjét
   ...
   // --------------------------------------------------------------------------
    ...
    ARFDATA.DAT strukturája:
      ...
      byte = Pénztárok darabszáma (ennyi iroda van)
         ...
         (CIKLUS: 1 To pénztárdarabszam:)
             ...
             xstring = 1. pénztár kódolt neve (xor 255) (első byte= hossz)
    ...
    // Url - adatai:
     ...
     byte = URL DARAB
       ...
       byte   = URL sorszáma
       XTRING = url kódolt neve (xor 255)(első byte= hossz)
       .........
    ...
    // Munkacsoportok adatai:
       ...
       Az összes munkacsoport, 27 valutájának 7 oszlopának 4 propertyje:
...
begin
  ...
  _adatverzio := Getbyte;       // Ez a verziószám   14 VAGY 15
  ...
  _qq := 1;
  _darab := 27;
  IF _adatverzio=14 then _darab := 24;
...
(*
function Tadatbetoltes.GetDnemDarab(_adpath: string): integer;

var _byte: byte;

begin
  AssignFile(_olvas,_adPath);
  Reset(_olvas);
  _byte := GetByte;
  CloseFile(_olvas);
  Result := 21;
  if _byte<128 then Result := _dnemdarab;
end;
*)
