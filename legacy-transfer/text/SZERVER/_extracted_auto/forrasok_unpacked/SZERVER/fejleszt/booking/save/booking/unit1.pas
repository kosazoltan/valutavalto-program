...
var
  ...
  _aktkeszpenz,_aktkartyas,_hufnyito,_hufkap,_hufad: real;
...
begin
  ...
  progressPanel.Caption := 'Az adatok leellenőrzése';
  progressPanel.repaint;
...
begin
  ...
  _nyitottIndex := 0;    // a müködő pénztárak indexe nullázva
  ...
  while _penztarindex<=_penztarDarab do  // az összes (bezártak is) pénztárt vizsgálja
    begin
      ...
      // Az exlusive - vagy -  express adatbázis útvonala:

      If _aktpenztar<151 then
         _Vpath := _host + ':C:\receptor\database\V'+inttostr(_aktpenztar)+'.FDB'
         else _Vpath := _host + ':C:\Cartcash\database\V'+inttostr(_aktpenztar)+'.FDB';

      _pts := null3(_aktpenztar);    // pl: '009','033','143' etc.

      // A szerver megfelelő adatbázisát nyitja meg:
      ...
      // Létezik a havi gyüjtőtábla - ezért neg is nyitja:
      ...
      while not RemoteQuery.Eof do
        begin
          ...
          // A konverziós forintot nem vesszük figyelembe !?
    ...
    begin
      ...
      while _x<=27 do
        begin
          ...
          inc(_x);     // következő valutanem indexe
        end;

      inc(_y);        // következő pénztár indexe
    end;
...
begin
  ...
  while _nyitottIndex<=_nyitottPenztardarab do
    begin
      ...
      // Amennyiben van előhavi cimtárban - akkor megnyitja
      ...
      if ibtabla.Exists then
        begin
          // Megállapítja az elöző hónap utolsó napját:
...
{
// =============================================================================
                procedure TForm1.NyitoZaroRegisztralas;  // ujabb
// =============================================================================
...
begin
  ...
  Csik.Max := _nyitottPenztarDarab;
  _nypenztarIndex := 1;
  while _nyPenztarIndex<=_nyitottPenztarDarab do
    begin
      Csik.Position := _nyPenztarIndex;
      _aktpenztar   := _nyitottpenztar[_nypenztarindex];
      _penztarIndex := ScanPenztar(_aktpenztar);
      _aktkorzet    := _korzet[_penztarIndex];
      _aktkft       := _kft[_penztarIndex];
      ...
      while _dnemIndex<=27 do
        begin
          _aktdnem      := _dnem[_dnemindex];
          _aktnyito     := _nyito[_nypenztarindex,_dnemindex];
          _aktzaro      := _zaro[_nypenztarindex,_dnemindex];

          prog3Panel.Caption := _aktdnem;
          prog3Panel.repaint;

          _ctrlnum := _aktnyito+_aktzaro;
          if _ctrlnum>0 then
            begin
              _pcs := 'SELECT * FROM TRANZAKCIOK' + _sorveg;
              _pcs := _pcs + 'WHERE (PENZTAR=' + inttostr(_aktpenztar)+') AND (';
              _pcs := _pcs + 'VALUTANEM='+chr(39)+_aktdnem+chr(39)+')';
              ...
              if _recno=0 then
                begin
                  _pcs := 'INSERT INTO TRANZAKCIOK (KFT,KORZET,PENZTAR,VALUTANEM,';
                  _pcs := _pcs + 'NYITO,ZARO,CSAKKESZLET)' + _sorveg;
                  _pcs := _pcs + 'VALUES (' + chr(39) + _aktkft + chr(39) + ',';
                  _pcs := _pcs + inttostr(_aktkorzet) + ',';
                  _pcs := _pcs + inttostr(_aktpenztar) + ',';
                  _pcs := _pcs + chr(39) + _aktdnem + chr(39) + ',';
                  _pcs := _pcs + floattostr(_aktnyito) + ',';
                  _pcs := _pcs + floattostr(_aktzaro) + ',';
                  _pcs := _pcs + '1)';
                end else
                begin
                  _pcs := 'UPDATE TRANZAKCIOK SET NYITO='+floattostr(_aktnyito);
                  _pcs := _pcs + ', ZARO='+floattostr(_aktzaro);
                  _pcs := _pcs + ', CSAKKESZLET=0' + _sorveg;
                  _pcs := _pcs + 'WHERE (PENZTAR='+inttostr(_aktpenztar)+') AND (';
                  _pcs := _pcs + 'VALUTANEM=' + chr(39) + _aktdnem + chr(39)+')';
                end;
              Bookparancs(_pcs);
