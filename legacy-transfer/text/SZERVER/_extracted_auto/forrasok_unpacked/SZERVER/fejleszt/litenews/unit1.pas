...
//  _plusText : string = 'ZÁLOG - ÉKSZER - MŰTÁRGY - CHANGE : ';
//  _plusbyte : byte = 36;
...
begin
  ...
  ArfolyamKikuldes;
  Memo.Lines.add('Inditom az idözítőt');
...
begin
  ...
  Memo.Lines.Add('Megtaláltam a '+_newRatesName+' nevű árfolyamfile-t');
  ...
  Memo.Lines.Add('Megpróbálom letölteni '+_newratesPath+' nevű file-t');
  ...
  if _sikeres then
    begin
      Memo.Lines.add('Sikeresen letöltöttem a '+_newRatesPath+' nevű file-t');
...
begin
  ...
  if _hFTP=nil then
    begin
      Memo.Lines.add('A központi sterver nem érhető el !');
      InternetCloseHandle(_hNet);
    end;
end;
...
begin
  Memo.Lines.add(_newRatesPath+' nevű file feldolgozása indul');
