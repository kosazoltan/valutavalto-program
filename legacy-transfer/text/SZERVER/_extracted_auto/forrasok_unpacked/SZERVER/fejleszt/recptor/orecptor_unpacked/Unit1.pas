...
var
  ...
  _napnev: array[1..7] of string = ('hétfő','kedd','szerda','csütörtök','péntek',
                     'szombat','vasárnap');
  ...
  _vPenztarNev    : array[1..150] of string;
  _zPenztarnev    : array[1..30]  of string;

  _vPenztarszam   : array[1..150] of byte;
  _zPenztarszam   : array[1..30]  of byte;
...
function daybookcontrol: integer; stdcall; external 'c:\receptor\dbctrl.dll' name 'daybookcontrol';  
function napimentesrutin: integer; stdcall; external 'c:\receptor\mentes.dll' name 'napimentesrutin';
function makeexclusivetablo: integer; stdcall; external 'c:\receptor\tablomake.dll' name 'makeexclusivetablo';
function makeexpressztablo: integer; stdcall; external 'c:\receptor\xtablo.dll' name 'makeexpressztablo';
...
begin
  ...
  KortombBetolto;             // a jelző körök tömbbe irása
  PenztarBeolvasas;
  Korclear;                   // A körök színe: fehér

  _szinPointer := 1;          // A 6 szin pointerre
  _aktszin     := _szinek[1]; // Kezdő kör színe: piros
  _count       := 0;
  _bigcount    := 150;
...
begin
  ...
  _maho := monthof(date);
  _munkahos := midstr(_munkanap,6,2);
  ...
  val(_munkahos,_munkaho,_code);

  if _munkaho<>_maho then daybookcontrol;  // havi nyito fdb-ek létrehozása
...
begin
  ...
  // Ha nem létezö a pénztár, akkor  ennyi !
   ...
   if _penztarTipus=0 then
     begin
       logirorutin(pchar('RECEPTOR = Nem létező pénztárszám'));
       UzenoPanel.Caption := 'NEM LÉTEZŐ PÉNZTÁRSZÁM !';
       Uzenopanel.Repaint;
       exit;
     end;
  ...
  // A CSOMAGFILE KITERJESZTÉSÉBŐL KIDERÜL A CSOMAG DÁTUMA
...
begin
  ...
  if _ptsz>150 then _dbkPath := 'c:\cartcash\database\daybook.fdb';

  Daybookdbase.close;
  daybookdbase.DatabaseName := _dbkPath;
...
begin
  ...
  _y := 0;
  while _y<_zPenztarDarab do
    begin
      if _zPenztarSzam[_y]=_ptsz then
        begin
          _aktPenztarNev := _zPenztarnev[_y];
          Result := 2;
          Break;
        end;
      inc(_y);
...
begin
  ...
  while not ArtcQuery.Eof do
    begin
      ...
      _zPenztarSzam[_cc] := _UZLET;
      _zPenztarNev[_cc]  := trim(ArtcQuery.FieldByname('KESZLETNEV').asString);
      inc(_cc);
      ArtcQuery.Next;
    end;
  ArtcQuery.close;
...
// =============================================================================
// $$$$$$$$$$$$$$$$$$$$  BIZALMAS BEJELENTES LEKEZELÉSE $$$$$$$$$$$$$$$$$$$$$$$$
// =============================================================================
...
var
    _confidb,_y: word;
    _confifile,_targfile,_targpath,_sourcePath: string;
    _ss: byte;
...
begin
  while True do
    begin
      ...
      if _confidb=0 then break;
      _y := 0;
      _ss := 65;
      while _y<_confidb do
        begin
          _confifile := _confi[_y]; //  CONF TRL A . COF
          _sourcePath := 'c:\receptor\mail\conf\' + _confifile;

          _targfile := leftstr(_confifile,7)+chr(_ss)+'.CXF';
          _targpath := 'c:\receptor\mail\conf\' + _TARGFILE;
          while true do
            begin
              if not Fileexists(_targpath) then break;
              inc(_ss);
              if _ss>90 then exit;
              _targfile := leftstr(_targfile,7)+chr(_ss)+'.CXF';
              _targpath := 'c:\receptor\mail\conf\' + _targfile;
            end;
          sysutils.RenameFile(_sourcePath,_targpath);
          inc(_y);
        end;
    end;
end;
...
// =============================================================================
  ...
  (* 8 nagybetüből álló stringből 1-999 közötti irodaszámot dekodol *)
...
begin
  if not FileExists('C:\RECEPTOR\MAIL\SCANS\ASK\A') then exit;

  _okmok := okmanymasolo;
  if _okmok=1 then _okmuz := 'F' else _okmuz := 'N';

  AssignFile(_txtiro,'c:\receptor\mail\scans\ask\'+_okmuz);
  rewrite(_txtiro);
  CloseFile(_txtiro);
end;
