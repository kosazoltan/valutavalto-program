...
var
  ...
  _srec: TsearchRec;
  _minta: string;
  _markernev: array[0..500] of string;
...
var _mNev,_veg: string;
    _ido: Tdatetime;
    _mfHossz: integer;

begin
  SIKERTELEN.Visible := FALSE;
  GetLetter.Repaint;
  _mResult := 2;
  _minta := _maildir + '\*.m';
    ...
    begin
      ...
      if Findfirst(_minta,faAnyFile,_srec)<>0 then
        ...
        begin
          ...
          _mfhossz := length(_munkafileNev);
          _veg := uppercase(midstr(_munkafilenev,_mfhossz-1,2));
          if _veg='.M' then
            begin
              deleteFile(_markerpath);
              Continue;
...
var _dnemsor,_bjegys,_munkatemp: string;
    _ukoke: boolean;
...
begin
  ...
  _munkaTemp := 'c:\receptor\temp\mtemp.dat';

  if Fileexists(_munkaTemp) then DeleteFile(_munkaTemp);
  CopyFileto(_munkaPath,_munkaTemp);

  // Megnyitja a bejegyzést:

  Assignfile(_olvas,_munkaTemp);
  Reset(_olvas);
  ...
  // Beolvassa, hány tételből áll a bizonylat:
 ...
 // A bizonylat tételek dekódolása:
  ...
  while _ss<_dnemdb do
    begin
      ...
      _ukoke := Form1.Ukserverelerheto;
      if not _ukoke then exit;
