...
begin
  ...
  if _recno=0 then
    begin
      ...
      Pgytabla.close;
      CLosefile(_iras);
      ShowMessage('NINCSEN RÖGZITETT HIBÁS TRANZAKCIÓ A PGYÜJTŐBEN');
  ...
  while not PgyTabla.Eof do
    begin
      ...
      // u-val kezdödő változókba (vagy nem sikerül result = false)
      ...
      if _vUgyfeltip='J' then
        begin
          ...
          //  elsőnek megkeresi a megbizott személy adatait:
    ...
    begin
      ...
      if _pont>1 then
        begin
          ...
          _megvan := True;
          writeLn(_iras,' ... tehát egyezőnek tekinthető');
          break;
        end;
...
begin
  ...
  if _recno=0 then
    begin
      // Nem találta meg a _vugyfelszamhoz tartozó rekordot (nem jellemző):
...

{
  ...
  ugyfélhez tartozó összes adatot u-val kezdödő valtozókba irja
}
...
begin
  ...
  // _u-val kezdödő változókba olvassa a natur adatokat

  _mm := 'Sikerült megtalálni '+inttostr(_vugyfelszam)+' ügyfélszámű ügyfelet';
  writeLn(_iras,_mm);
...
begin
  ...
  // A Jogiszemélynév első 15 karaktere alapján keresem:
...
begin
  ...
  writeLn(_iras,'Kiolvassa '+_lastmezo+' mező értékét és eggyel növeli: '+inttostr(_natursss));
...
begin
  ...
  writeLn(_iras,'A megbizott sorszama '+_lastmezo+' mező alapján: '+inttostr(_mbsss));
