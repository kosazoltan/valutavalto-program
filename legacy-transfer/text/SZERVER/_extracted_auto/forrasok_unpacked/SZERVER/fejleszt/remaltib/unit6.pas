...
begin
  ...
  if _akttipus='SZÖVEG' then
    begin
      Filledit.Left := 88;
      FillEdit.Width := 281;
      TipInfoLabel.Caption := 'A VÁLASZTOTT MEZŐ EGY SZÖVEGES ADAT';
      ILabel.Left := 80;
      ILabel.Caption := 'A MEZŐ FELTÖLTÉSE (TÖRLÉSE)AZ ALÁBBI SZÖVEGGEL';
      _tip := 'C';
    end else
    begin
      Filledit.Left := 150;
      FillEdit.Width := 145;
      TipInfoLabel.Caption := '   A VÁLASZTOTT MEZŐ EGY SZÁM ADAT';
      ILabel.Left := 120;
      ILabel.Caption := 'A MEZŐ FELTÖLTÉSE AZ ALÁBBI SZÁMMAL';
      _tip := 'N';
    end;
