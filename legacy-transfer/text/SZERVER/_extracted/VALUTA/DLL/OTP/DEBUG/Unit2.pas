...
//         POS-terminál elérhetőségének ellenörzése:
...
begin
  ...
  TranzCimPanel.caption := 'A POS TERMINÁL KOMMUNIKÁCIÓ ELLENŐRZÉSE';
  ...
  // Ctrl-uzenet kiküldése a terminálhoz.
     // A kiküldendő parancs összeállitása
...
begin
  if _fizetendo=0 then
    begin
      ShowMessage('Nincs kifizetendő összeg');
      _Mresult := 2;
      Kilepo.Enabled := true;
      exit;
    end;
...
begin
  ...
  while _pp<=_wm do
    begin
      ...
      // Ha az aktuális karakter (_asc) nem vezérlőkarakter, akkor az adat betüje
      if _asc>31 then
        begin
  ...
  if _sendA<>'A'+_aMezo then
    begin
      _subhmess := 'NEM EGYEZIK A PARANCS MEZŐ';
      _ujra := true;
      exit;
    end;
  ...
  if _sendY<>'' then
    begin
      if _sendY<>_Ymezo then
        begin
          _ujra := true;
          _subHmess := 'NEM EGYEZIK AZ Y-mező';
          exit;
        end;
    end;
...
begin
  ...
  case _hibaKod of
     ...
     9: result := 'Küldött string nem jelenithető meg';
...
begin
  ...
  case _gkod of
     ...
     0: result := 'A kezelő a kártyabehúzást megszakította';
     1: result := 'A kezelő az összeg megadását megszakította';
     2: result := 'A kezelő vagy az ügyfél az összeget  nem hagyta jóvá';
     3: result := 'Az ügyfél a PIN megadását megszakította';
     4: result := 'A kezelő a számlatípus megadását megszakította';
     5: result := 'Az ügyfél a számlatípust nem hagyta jóvá';
     6: result := 'A hivatkozás megadását megszakította';
     7: result := 'A kezelő a TeleBank – Házibank tranzakció adatait nem adta meg';
     8: result := 'A kezelő az engedélyszám megadását megszakította vagy időtúllépés';
    10: result := 'A kártyabehúzásra szánt idő alatt a kártyabehúzás nem történt meg';
    11: result := 'Az összeg megadására szánt idő alatt a megadás nem történt meg';
    12: result := 'Az összeg jóváhagyására szánt idő alatt a jóváhagyás nem történt meg';
    13: result := 'A PIN megadására szánt idő alatt a megadás nem történt meg';
    14: result := 'A számlatípus megadására szánt idő alatt a megadás nem történt meg';
    15: result := 'A számlatípus jóváhagyására szánt idő alatt a jóváhagyás nem történt meg';
    16: result := 'A hivatkozás jóváhagyására szánt idő alatt a jóváhagyás nem történt meg';
...
begin
  ...
  case _gkod of
    ...
    52: result := 'Előző tranzakció még nem zárult le';
  end;
end;
...
begin
  ...
  result := True;  // Nem vizsgálom az LRC-t
  {
