...
// ----------------------------------------------------------------------------
// VTEMP-ből beolvassa   : _konverzio
...
// Megállapitja _bizelokod és _ezkonverzio értékét
  ...
  //Az aktuális évtized megűllaőitása:
  _aktdek  := yearof(Date)-2000;

  // Az ugyfelyy.fdb és kisugyfel.fdb elérési útvonalámak megállaőítása:
  ...
  // ------------------------------------------------------------------
  // A konverzió-flagtől függöen, kiirja vagy nem, hogy konverziós vásárlás:
  ...
  // -----------------------------------------------------------------------
  // Elsó valuta bekérésre előkészitve:
...
// =============================================================================
      procedure TVasarlasForm.DnemKeyDown(Sender: TObject; var Key: Word;
                                                         Shift: TShiftState);
// =============================================================================
//           A valutanem mezőbe irt egy betűt:
...
begin
  ...
  if _bill=13 then
    begin
      ...
      // Itt rendben van a valuta neve - beirja a megfelelő tömbbe:
      ...
      _wa[_aktsor].Enabled    := True;   // Az aktuális sor arfolyam mezője
      _wb[_aktsor].Enabled    := True;   // és bankjegy mezője megnyitva
  ...
  If _bill=38 then
    begin
      // A legfelső sorról nincs felfele:
...
// =============================================================================
      procedure TVasarlasForm.BankjegyKeyDown(Sender: TObject; var Key: Word;
                                                         Shift: TShiftState);
// =============================================================================
//           A bankjegy mezőbe irt egy számot:
...
begin
  ...
  if _bill=13 then
    begin
      ...
      // A tényleges árfolyam kiolvasása a sor tömbjéből:
      _aktarfolyam     := _wArfolyam[_aktsor];

      // A sor forintértékének kiszámolása a valutanemtől függően:
      ...
      if _tetel=_aktsor then
        begin
          ...
          // A következő sor valutanem oszlopát teszi aktuálissá:
  ...
  if _bill=37 then
    begin
      ...
      // HA be van irva már a bankjegy, akkor a sor árfolyam mezője lesz aktiv:
  ...
  if _bill=38 then
    begin
      // Legfelső sorról nincs fellépés:
      if _aktsor=1 then exit;

      // Az eggyel feljebbi sor bankjegy mezője lesz az aktiv edit:
      ActiveControl := _wb[_aktsor-1];
      exit;
    end;
  ...
  if _bill=40 then
    begin
      ...
      // Ha van még lejebb is sor - akkor annak bankjegy mezőjére lép:
      if _tetel>_aktsor then
        begin
          ActiveCOntrol := _wb[_aktsor+1];
          exit;
      ...
      if (_tetel=_aktsor) and (_aktsor<6) then
        begin
          ...
          // HA a legalsó sor aktiv és még lehet újabb sor a következő sor Valutanem-Editje lesz aktiv
...
// =============================================================================
      procedure TVasarlasForm.ArfolyamKeyDown(Sender: TObject; var Key: Word;
                                                         Shift: TShiftState);
// =============================================================================
//             Az árfolyam mezőre billentyüt nyomott
...
begin
  ...
  if _bill=13 then
    begin
      ...
      // Ha már van a soron engedmény - itt nincs több lehetőség:
  ...
  if _bill=38 then
    begin
      ...
      // Az aktuális az eggyel feljebbi sor arfolyam mezője::
    ...
    begin
      ...
      if _tetel>_aktsor then
        begin
          ...
          // Az aktuális edit a következő sor árfolyamoszlopa:
          ActiveCOntrol := _wa[_aktsor+1];
          exit;
        end;
      ...
      if (_tetel=_aktsor) and (_aktsor<6) then
        begin
          // Az aktuális mezö a következő sor (a legalsó) valutanem oszlopa lesz:
          Activecontrol := _wd[_aktsor+1];
          exit;
        end;
    end;
...
begin
  ...
  // A visszatérő minusz 1 jelzi, hogy mégsem ad árfolyamkedvezményt:
...
begin
  // A VTEMP-ből kiolvassa: ARFOLYAMOT,BANKJEGYET,KEDV.ARFOLYAMOT,SORENGEDMENYT
  // a kiolvasott adatokat a megfelelő valutanemek tömbjeibe irja, miközben
...
begin
  ...
  KezdijengedmenyGomb.Enabled := False; // Nincs lehetőség kezdijengedmény adásra
  ...
  // A lehetséges árfolyam kedvezmény engedélyezőjének lekezelése:
...
begin
  ...
  // A VTEMP-be irja a megfelelő árfolyam engedély tipusát = 16m32m64 vagy 128
  ...
  case _rateEngKod of
    ...
    32 : _etips := ' főértéktárosi';
    64 : _etips := ' ügyvezetői';
...
//  itt van vége a 2-ik fázisnak:
  ...
  // ===========================================================================
  //////////////////////////////////////////////////////////////////////////////                                                                          //
  //////////////////   ITT VÉGLEGESITHETŐ A TRANZAKCIÓ   ///////////////////////
  //////////////////////////////////////////////////////////////////////////////
  // ===========================================================================
  ...
  // ------------------------------------------
  // Mégsem endedélyezi a tranzakciót a pénztáros . Vissza a főmenüre
...

{
     Kedvezmény word:
        ...
        LOW BYTE:  1 - F1 SÁVOS NORMÁL KEDVEZMÉNY
                  ...
                  32 -        - FŐÉRTÉKTÁROSI ENGEDMÉNY    JELSZAVAS
                  64 -        - ÜGYVEZETŐI ENGEDMÉNY       JELSZAVAS
        ...
        HIGH BYTE:  256 - KEZELÉSI DÍJ GOMBBAL FELEZÉSE            JELSZAVAS
                   ...
                   2048 - ÜGYVEZETŐ JELSZAVAS ELENGEDÉS            JELSZAVAS
...
// =============================================================================
//
// Üzenet az árfolyamkijelzőnek: Trja ki, hogy számolja meg a pénzét !
//
...
begin
  ...
  if _serverAccess=1 then
    begin
      Showmessage('SZERVER NEM ELÉRHETŐ');
      exit;
    end;
...

{
     ...
     A szamla adatait be kell dolgozni a szerveren lévő UGYFELyy adatbázisba
...
begin
  ...
  // Kiolvassa a távoli szerver NEVTABLA/SORSZAM sorában lévő adatokat:
...
begin
  ...
  _pcs := _pcs + ',NUMBER=7';
  ValutaParancs(_pcs);
  logirorutin(pchar('Elinditja a QR-kód kijelző rutint'));
  qrdisplayrutin;
end;
