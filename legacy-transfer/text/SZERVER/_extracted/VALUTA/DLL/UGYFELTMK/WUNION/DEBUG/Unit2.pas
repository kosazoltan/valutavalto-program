...
begin
  ...
  if _homepenztarszam<151 then
    begin
      ...
      if _kftnev='BEST' then _cegadoszam := '11788670-2-41';
      if _kftnev='EAST' then _cegadoszam := '23008975-2-41';
      if _kftnev='PANNON' then _cegadoszam := '23009017-1-41';
...
begin
  if (_foegyseg=1) then
    begin
      if _tranzakcio='+B' then _tranztipusnev := 'Ellátmány átvétele Exlusive Cashtől'
  ...
  if (_foegyseg=5) then
    begin
      if _tranzakcio='+B' then _tranztipusnev := 'Pénzátvétel egy ügyféltől'
...
begin
  ...
  if _igdat<_toldat then
    begin
      ShowMessage('A kezdőnap nagyobb az utolsó napnál');
      exit;
    end;
