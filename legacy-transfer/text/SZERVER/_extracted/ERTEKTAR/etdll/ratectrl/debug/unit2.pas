...
var
  ...
  _etarnames: array[1..8] of string = ('SZEKSZÁRD','SZEGED','KECSKEMÉT','DEBRECEN',
...
begin
  ...
  if _ertekTarszam=0 then
    begin
      ShowMessage('EZ AZ IRODA NEM ÉRTÉKTÁR !');
      Kilepo.enabled := true;
      exit;
    end;
...
begin
  ...
  if (_kertev=_maiev) and (_kertho>_maiho) then
    begin
      ShowMessage('A KÉRT HÓNAP A JÖVŐBEN LESZ');
      Messpanel.Visible := false;
      exit;
    end;
  ...
  if not _engoke then
    begin
      Showmessage('NEM VOLT ENGEDÉLY ADÁS A KÉRT H
     exit;
  }
          ...
          while not RemoteQuery.Eof do
            begin
              ...
              if _arfelo>0 then
                begin
                  case _arfelo of
                     16: _pengedo := 'ÉRTÉKTÁROS';
                     32: _pengedo := 'FŐÉRTÉKTÁROS';
                     64: _pengedo := 'ÜGYVEZETŐ';
                    128: _pengedo := 'JUTALÉKMENTES';
                  end;
              ...
              if _arfehi>0 then
                begin
                  ...
                  if _arfehi=4 then _pengedo := 'ÜGYVEZETŐ';
...
begin
  ...
  result := '';
  if _eb='J' then result := 'JUTALÉKMENTES';
end;
