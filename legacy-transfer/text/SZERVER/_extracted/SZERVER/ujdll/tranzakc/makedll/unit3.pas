...
type
  TMNBARFOLYAM = class(TForm)
    ...
    function HunstrtoDate(_ds: string): TDatetime;
...
begin
  ...
  while _nn<=_houtolsonap do
    begin
      ...
      _aktdatum  := hunstrtodate(_aktdatums);
...
// Feladata: A képernyőn látható féltáblázat tömbbe (datatomb) beirása:
...
function TMNBArfolyam.HunstrtoDate(_ds: string): TDatetime;

var _yevs,_yhos,_ynps: string;
    _yev,_yho,_ynp: word;

begin
  _yevs := leftstr(_ds,4);
  _yHos := midstr(_ds,6,2);
  _yNps := midstr(_ds,9,2);

  val(_yevs,_yev,_code);
  val(_yhos,_yho,_code);
  val(_ynps,_ynp,_code);

  result := Encodedate(_yev,_yho,_ynp);
