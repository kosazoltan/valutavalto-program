...
(* -------- Itt kezdödik a napi forgalmi adatok bedolgozása -----------------*)
  ...
  while not BlokkQuery.Eof do
    begin
      ...
      // AZ ELSŐ TÉTEL (BLOKKFEJ) LERENDEZÉSE:
      if (_bizonylatszam<>_utbiz) then
        begin
          ...
          // A VALUTA ELADÁS LERENDEZÉSE AZ ELSŐ TÉTELMÉL
          if _tipus='E' then
            begin
...
begin
  ...
  while not ReceptorQuery.Eof do
    begin
      ...
      _mondat := _aktdnem + chr(9) + inttostr(_eladott);       //  1-2 mzező
      _mondat := _mondat  + chr(9) + inttostr(_vett);          //  3-ik mező

      _mondat := _mondat  + chr(9) + inttostr(_bkadott);       //  4-ik mező
      _mondat := _mondat  + chr(9) + floattostr(_bkforint); //  5 -ik mező

      _mondat := _mondat  + chr(9) + inttostr(_bkkezdij);      //  6-ik mező
      _mondat := _mondat  + chr(9);                            //  7-ik mező
      _mondat := _mondat  + chr(9) + inttostr(_kpkezdij);      //  8-ik mező
...
{
   Meglévő adatok:  _aktpenztar (1..168)    closed='N'
                    _sktstatus  ('B' or 'X')
