...
var
  ...
  _xMezo          : array[1..169] of string;
  _xmTip          : array[1..169] of byte;
  ...
  _mResult,_aktnum,_iIndex,_aktugyfelszam: integer;
...
begin
  ...
  _dataPath := 'c:\receptor';
  if _aktpenztarszam>150 then _dataPath := 'c:\cartcash';
  ...
  _aktfdbPath := _dataPath + '\database\v' + inttostr(_aktpenztarszam)+'.fdb';
  ...
  if (_versid<>'NX') then
    begin
      _mess := 'Nem megfelelő program-id='+_versid;
      Memo1.Lines.add(_mess);
      Closefile(_binolvas);
      Kilepo.enabled := True;
      exit;
  ...
  CloseFile(_binolvas);
   ...
   Memo1.Lines.Add('A NAPI ZÁRÁS ADATAIT RENDBEN BEDOLGOZTAM');
  _mResult := 1;
