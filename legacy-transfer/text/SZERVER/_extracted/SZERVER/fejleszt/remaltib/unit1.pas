...
var
  ...
  _relacio: array[0..5] of string = (' NAGYOBB',' NAGYOBB/EGYENLŐ',' EGYENLŐ',' KISEBB/EGYENLŐ',
                                       ' KISEBB','NEM EGYENLÖ');
...
begin
  ...
  TablaLista.ItemIndex := 0;  // Kurzor a legelső táblanéven
...
begin
  ...
  // A keresés mezőtőmbjét a comboba másolja az esetleges kereséshez:
  KermezoCombo.Clear;
  ...
  // Az IBtabla első rekordját beolvassa  a MEZODATA tömbbe:
  IBTabla.First;
  RekordRead;

  // Maximum a tábla első 8 adatának beirása az esetleges Táblás kijelzéshez
  TablaBeiras;
end;
...
begin
  ...
  _pcs := _pcs + 'WHEN 13 THEN ' +CHR(39)+ 'IDŐ' + CHR(39)+_sorveg;
...
begin
  ...
  // Ha a mezö csak 8 vagy kevesebb: A legalsó használt panel: ahány mező van
  ...
  // A felső mező sorszáma: 1, a legalsó (ha van) 8
  _felsoAdat := 1;
  _alsoAdat := 8;

  // Az aktiv mezö sorszáma=1 ( a legfelső)
  _AktMezoSorszam := 1;
end;
...
begin
  ...
  // Az aktuális mező sorszámát igy számitom ki:
...
begin
  (* A 2 paraméter: Rekord Mezősorszáma és az Edit sorszáma *)

  // Default: nem helyes a bevitt adat:
  Result := false;

  // Az aktuális mező értéke:
  _aktData := trim(_mezoData[_mss]);

  // A kötelező adathossz:
  _aktMezoHossz := _mezoHossz[_mss];

  // Az aktuális mező tipusa:
  _aktMezoTip := _mezoTipus[_mss];

  // Az aktuális mező adatának EDIT-je:
  ...
  if _aktmezoTip= 'SZÖVEG' then
    begin
      ...
      // Az új adat bekerül az mezőadattömbbe
  ...
  if (_aktmezoTip='EGÉSZ SZÁM') then
    begin
      ...
      // Az uj adat bekerül a mezőadattömbbe:
      _mezodata[_mss] := _aktText;
      Result := True;
      exit;
    end;
...
// ------ Az indexelendő mezők kiemelése ....
...
// Az index információk kijelzése ....
  ...
  if Csokkenes.Checked then TendenciaPanel.Caption := 'CSÖKKENŐ'
  else TendenciaPanel.Caption := 'NÖVEKEDŐ';
