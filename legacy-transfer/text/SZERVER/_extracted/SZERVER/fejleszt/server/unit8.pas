...
// =============================================================
   (*
        ...
        _DNEM[0.._valutadarab-1] = az árfolyam.dbf-ből vett DNEM
        _DNEV[0.._valutadarab-1] = az árfolyam.dbf-ből vett DMEGN
        _VARF[0.._valutadarab-1] = az árfolyam.dbf-ből vett VARF
        _EARF[0.._valutadarab-1] = az árfolyam.dbf-ből vett EARF
        _ELSZARF[0.._valutadarab-1] = az árfolyam.dbf-ből vett ELSZ_ARF
        _MODISZ[0.._valutadarab-1] = az árfolyam.dbf-ből vett MODISZAZ
        -------------------------------------------------------------
                          CIMLET.CFG ALAPJÁN:

         _CONFIGSS = ennyi valutának van meg a paramétere
         _VNEM[1.._CONFIGSS] = a setupban lévő valutanevek
         _VCDB[1.._CONFIGSS] = a setupban lévő valuták elérhető cimletei
         ...
         --------------------------------------------------------------------
          ...
          _LIVECIMBANKJEGY[1.._liveValutaDarab,1..14] = egyes valuták cimlet-
                                               darabszámai
          _LIVEoSSZES[1.._liveValutaDarab] = Az élő valuták cimletezett
                                               értékei
...
// -------------------- Az élő valuták cimleteinek beolvasása -------------
...
begin
 ...
 // A cimtárgyüjtő neve és útvonala meghatározása:
   ...
   If _vtag>0 then
     begin
       ...
       for i := 1 to 14 do
         ...
         begin
           _aktcbjegy := _cic[i];     // A beolvasott bankjegy ebből a cimletböl
           _aktertek := _cimlet[i]*_aktcbjegy;  // A fenti értéke
           _liveCimBankjegy[_vtag,i] := _aktCBjegy;
           _scim := _scim + _aktertek;
         end;
       _liveOsszes[_vTag] := _scim;  // ennyi érték van ebből a valutából
     end;
end;
