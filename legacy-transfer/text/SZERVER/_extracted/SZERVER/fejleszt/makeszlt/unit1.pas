...
var
  ...
  // Körzetben lévő pénztárak:

  _korzetptdb      : array[1..9] of integer;        // ennyi ptár adata jött be körzetenként
  _korzetptsor     : array[1..9,1..40] of integer;  // körzetekben lévő irodaszámok

  _foglalodnem   : array[0..4] of string = ('HUF','CHF','EUR','GBP','USD');

  // Pénztárak adatai:

  _ptKeszlet     : array[1..150,1..27] of integer; // Az élőirodák valutakészletei
  ...
  _acKeszlet     : array[0..20,1..27] of integer;  // Az élőirodák valutakészletei
...
begin
  ...
  CimPanel.Caption := 'ADATOK LETÖLTÉSE A SZERVERRŐL';
  Cimpanel.Repaint;
  ...
  if not KeforLetoltes then
    begin
      ShowMessage('NEM SIKERÜLT EGYETLEN ADATFILET SEM LETÖLTENI A SZERVERRŐL');
      Application.Terminate;
      exit;
    end;
  ...
  // ---------------- A cégösszesitő táblája miatti változók  ------------------
...
begin
  ...
  _ktsznev: 'pk128.120  = 120 ÉRTÉKTÁR, 128-AS PÉNZTÁR
   ...
   - ciklus indul 1-től valutaDarabByte-ig
...
//  Az adatok alapértékre állitása a munkát megelözően:
...
begin
  ...
  _oxl.Activewindow.FreezePanes := True;
   ...
   Alcimpanel.Caption := 'A NYOLC KÖRZET TÁBLÁJÁNAK ELŐKÉSZÍTÉSE';
   AlcimPanel.Repaint;
    ...
    begin
      ...
      while _valutaIndex<=27 do
        begin
          ...
          inc(_valutaIndex);   // Jöhet a következő valutanem
        end;
      ...
      // A WU- ÁFÁ KEZDIJ EKER FOGLALOK cellák előkészitése:
      ...
      inc(_ptsorIndex); // Jöhet a következő pénztár a sorban
    end;
...
begin
  ...
  // A legfelső magyarázó főcimsor beállitása:
  ...
  _rangestr := 'A7:B7';
  _oxl.worksheets[_korzetIndex].Range[_rangestr].Mergecells := True;
  _oxl.worksheets[_korzetIndex].Cells[7,1] := 'IDŐ';
  ...
  while _ptsorindex<=_aktkorzetDarab do
    begin
      // Az első a 3-ik oszlop: (majd 5,7 stb)

      Fastcsik.Position := _ptsorIndex;
      _aktoszlop := 1 + trunc(2*_ptsorindex);

      // A következö pénztárszám:  ELSŐ RANGE= 'C4:B4'
      ...
      // ------------- Elsö range = 'C6:D6' Beirja az aktuális dűtumot ---------
...
begin
  ...
  begin
    ...
    repeat
      ...
      // A következő megtalált Kefor file forrása és céja:
...
begin
  ...
  while _valutaIndex<=27 do
    begin
      ...
      inc(_valutaIndex); // jöhet a következő valuta:
    end;
