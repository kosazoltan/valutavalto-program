...
type
  TAdatSzetkuldes = class(TForm)
    ...
    procedure OldDataFileWrite;
    procedure ujDataFileWrite;

    procedure RemoteFileDelete;

    function UploadFiles(_local: integer): boolean;
...
begin
  ...
  _oldDataPath := _workdir + '\olddata.dat';
  ...
  // Elkésziti a RÉGI TIPUSU FR - filet  (...... byte hosszú)
  oldDatafileWrite;

  // Elkésziti az újtipusú NR - filet
  ujDataFileWrite;
  ...
  _pecsOke  := False;
  _BcsabaOke:= UpLoadFiles(1);
  if _kellPecs then _PecsOke := UploadFiles(2) else _pecsOke := true;

  if (not _bcsabaOke) and (not _pecsOke) then
    begin
  ...
  if (_bcsabaoke) and (not _pecsoke) then
    begin
      _mresult := 2;
      ErrorPanel.Caption := 'CSAK A BÉKÉSCSABAI SZERVERRE TUDTAM IRNI';
      ErrorPanel.Visible := true;
      ErrorPanel.Repaint;
      Sleep(2000);
      Modalresult := 2;
  ...
  if _kellpecs then
    begin
     if (not _BCSABAoke) and (_Pecsoke) then
        begin
          _mresult := 3;
          ErrorPanel.Caption := 'CSAK A PÉCSI SZERVERRE TUDTAM IRNI';
          ErrorPanel.Visible := true;
          Errorpanel.Repaint;
...
begin
  {
    ...
    54 csoport= 27 valutanem - 9 munka oszlopa - 5 byton = 1215
                + 6 byteon a 3 limit
                1 csoport összesen = 1221 byte
    Összes csoport összesen: 54 * 1221 = 65934 byte

    Teljes file : 1 (verzió) + 200 (csoport) + 65934 (árfolyam+limit) + 3 byte (záró)

    Teljes file hossz = 66138

    Csoport kezdöbyte= 201 + (csoport-1)*1221
  ...
  while _aktcsoport<=54 do
    begin
      ...
      ArfolyamKiiro(27); // 27 valutanem 7-7 árfolyamát irja ki !  byte
  ...
  // A file végét jelző 3 byte kiirása:
...
// =============================================================================
             procedure TadatSzetkuldes.OldDataFileWrite;
// =============================================================================
...
begin
  {
    arfolyamfile neve = 'RFddhhmm.dat'

    1 Byte   = verzió  = 14
    200 byte = pénztárak csoportkódja
    54 csoport= 24 valutanem - 9 munka oszlopa - 5 byton = 1080
                + 6 byteon a 3 limit
                1 csoport összesen = 1086 byte
    Összes csoport összesen: 54 * 1086 = 58644 byte

    Teljes file : 1 (verzió) + 200 (csoport) + 58644 (árfolyam+limit) + 3 byte (záró)

    Teljes file hossz = 58848

    Csoport kezdöbyte= 201 + (csoport-1)*1086
  ...
  _oldDataName := 'RF' + nulele(_nap)+ leftstr(_aktidos,2)+midstr(_aktidos,4,2)+'.DAT';

  Assignfile(_iras,_OldDataPath);
  Rewrite(_iras);

  _bytetomb[1] := 14;  //   14-es verzió
  Blockwrite(_iras,_bytetomb,1);
  ...
  while _aktcsoport<=54 do
    begin
      ...
      ArfolyamKiiro(24); // 24 valutanem 7-7 árfolyamát irja ki !
...
// =============================================================================
        function TadatSzetKuldes.UploadFiles(_local: integer): boolean;
// =============================================================================
...
begin
  ...
  _localPath  := _oldDataPath;
  _remoteFile := _olddataName;
...
begin
  ...
  _hSearch := FtpFindFirstFile(_hFtp,'RF*.DAT',_findData,0,0);
