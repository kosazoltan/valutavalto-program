...
type
  TIMPORTFORM = class(TForm)
    ...
    procedure Lakcimbonto(_s: string);
...
var
  ...
  _szulhely,_anyjaneve,_okmtipus,_azonosito,_joginev,_telephely,_lakcim: string;
  ...
  _mbTabla,_fdbpath,_aktceg,_mbData,_msorszams,_mbelonev,_teaor: string;
  _bizszam,_utip,_ugyfnev,_kbetu,_okmtip: string;
  _lakcimtelep,_lakcimutca,_cegtelep,_cegutca: string;

  _akttelep,_aktutca: string;
...
function ujimportkeszito: integer; stdcall;
...
begin
  ...
  if _kertdatum>date then
    begin
      ShowMessage('A KÉRT DÁTUM MAJD A JÖVŐBEN LESZ');
      exit;
    end;
...
begin
  ...
  _pcs := _pcs + 'TELEPHELY,TELEPHELYCIM,OKIRATSZAM,MBNEVE,MBELOZONEV,MBSZULIDO,';
  _pcs := _pcs + 'MBSZULHELY,MBANYJANEVE,MBISO,MBOKMTIPUS,MBAZONOSITO,';
  _pcs := _pcs + 'ALLANDOTELEPULES,ALLANDOUTCA,TEAOR,KFT)' + _sorveg;
  ...
  _pcs := _pcs + chr(39) + _cegtelep + chr(39) + ',';
  _pcs := _pcs + chr(39) + _cegutca + chr(39) + ',';
  ...
  if _ugyftipus<>'cég' then
    begin
      if trim(_lakcimtelep)='' then _lakcimtelep := 'ISMERETLEN';
      if trim(_lakcimutca)='' then _lakcimutca := 'ISMERETLEN';
    end;

  _pcs := _pcs + chr(39) + _lakcimtelep  + chr(39) + ',';
  _pcs := _pcs + chr(39) + _lakcimutca + chr(39) + ',';
...
begin
  ...
  LakcimBonto(_lakcim);
  _lakcimtelep := _akttelep;
  _lakcimutca := _aktutca;
...
begin
  ...
  with UgyfelQuery do
    begin
      ...
      _teaor      := trim(Fieldbyname('TEAOR').AsString);
      _mbData     := trim(FieldByName('MBDATASORSZAM').asstring);
    end;

   Lakcimbonto(_telephely);
   _cegtelep := _akttelep;
   _cegutca  := _aktutca;
        ...
        begin
          ...
          with Impquery do
            begin
              ...
              _azonosito        := trim(FieldByNAme('AZONOSITO').AsString);
              _lakcimtelep      := trim(FieldByName('ALLANDOTELEPULES').AsString);
              _lakcimutca       := trim(FieldByNAme('ALLANDOUTCA').asString);
              _joginev          := trim(FieldByNAme('JOGINEV').AsString);
              _jiso             := fieldByNAme('JISO').AsString;
              _cegtelep         := trim(Fieldbyname('TELEPHELY').AsString);
              _cegutca          := trim(FieldByNAme('TELEPHELYCIM').AsString);
              _okiratszam       := trim(fieldByNAme('OKIRATSZAM').AsString);
          ...
          write(_iras,_lakcimtelep+';'); // csak város mo irszam
          write(_iras,_lakcimutca+';');  // utca + házszám

          write(_iras,_joginev+';');
          write(_iras,_jiso+';');

          write(_iras,_cegtelep+';');  // csak a város no irszám
          write(_iras,_cegutca+';');   // csak utca és házszám
          write(_iras,_teaor+';');           // teaor kód
...
begin
  ...
  write(_iras,'azonosító okmány száma;állandó lakcím település;');
  write(_iras,'állandó teljes lakcím;cég neve;bejegyzés országa;');
  write(_iras,'székhely település;székhely teljes cím;TEÁOR kód;');
...
procedure TImportform.lakcimbonto(_s: string);

var _ws,_pos,_elsobetukod: byte;
...
begin
  ...
  _elsobetukod := ord(_s[1]);
  if (_elsobetukod<58) and (_elsobetukod>47) then  // ez irányitószám
    begin
      _pos := Pos(chr(32),_s);
      if _pos<6 then _s := midstr(_s,_pos+1,_ws-_pos);
    end;
  _ws := length(_s);
  _pos := pos(chr(32),_s);
  _akttelep := leftstr(_s,_pos-1);
  _aktutca  := midstr(_s,_pos+1,_ws-_pos);
end;
