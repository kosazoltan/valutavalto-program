...
//  _nvdb,_nedb: array[1..150,1..31] of integer; // napi vételek/eladások száma boltonként
  ...
  _nPros: array[1..150,1..31] of byte;  // Irodánként naponta, a főpénztáros sorszáma

  _utDatum,_utBlokknum: string;  // ciklus váltás figyelő változók
  _aNapok: byte;

  // -------------------- Elözö havi adatok --------------------------------------

  _ehNapok: array[1..150] of byte;  // Elöző havi munkanapok irodánként
  _ehVasar,_ehEladas: array[1..150] of integer; // elözö havi eladások és vásárlások ft*ban, irodánként
  _ehVdb,_ehEdb: array[1..150] of word; // elöző havi vevő és eladó ügyfél darabszáma irodánként
  ...
  _tablonap: word; // A tablón szereplő havi utolsó nap
...
begin
  ...
  // Mehet az összes körzeten át ívelő ciklus:
...
// Feltölti a következő tömböket: _irodanev[irodaszam]
//                                _irodaErtektar[irodaszam]
//                                _korzetirodadarab[1..8]
//                                _korzetirodasor[1..8,1..korzetirodadarab]
//
//  Beállitja a következő változót: _irodadarab;
//  ----------------------------------------------------------------------------
...
begin
  ...
  // Megnézi az iroda eddigi pénztáros sorát (Max 8 fő)
...
begin
  ...
  if not ibTabla.Exists then
    begin
      ...
      // Ha elöző havi cimletek sincsenek, akkor nincs készlet !!
...
BEGIN
   ...
   // -------------- Az értéktárban lévő irodák száma ----------------------
         ...
         begin
           ...
           if (_napivetel>0) or (_napieladas>0) then
             begin
               ...
               // Főpénztáros sorszama az adott napon
