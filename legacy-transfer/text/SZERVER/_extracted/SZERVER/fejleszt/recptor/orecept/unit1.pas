...
begin
  ...
  Figyelo.Enabled       := False;  // a tényleges figyelő szolgálat
  ...
  //  AZ UTOLSÓ TÁBLÁZATLAN (LEZÁRATLAN) NAP LÁTSZIK A KÉPERNYŐN
...
begin
  ...
  // Ha nem létezö iroda, akkor a markerfile-t törölni kell, és ennyi !
   ...
   if not _penztarOke then
     begin
       MemoTabla.Lines.Add('NEM LÉTEZŐ PÉNZTÁRSZÁM !');
    ...
    begin
       ...
       while true do
          begin
            ...
            // Mezönév = a tábla aktuális mező neve
            _mezonev := xUjmezo[_mezoss];

            // Mezökod = Az aktuális mező kódolása: 1-11 között
            _mezokod := xUjkod[_mezoss];

            // Mezőtip = Az aktuális mező tipusa: Integer, Float, String
            _mezotip := Xtipusok[_mezokod];

            // A kód függvényében beolvassa az aktuális adatot:
            _mAdat := Dataread(_mezokod);

            // A mezőtipustól függöen beirja a táblába az aktuélis adatot
            if _mezotip='I' then
...
begin
  case _kod of
     ...
     7: begin
          (* 5 byte-on kódolt lebegőpontos szám *)
    ...
    11: begin
          (* 5 byte-on kódolt lebegőpontos szám egesz resze *)
          BlockRead(_olvas,_bytetomb,5);
          _kodszo := chr(_bytetomb[1]) + chr(_bytetomb[2]) + chr(_bytetomb[3]);
          _kodszo := _kodszo + chr(_bytetomb[4])+chr(_bytetomb[5]);
          _float := IntegDek(_kodszo);
...
(* Három karakterből álló stringből egy Blokkszámot dekódol *)
...
(* Két karakterből álló stringből 3 betüs valutanevet generál *)
...
(* Öt karakteres stringből lebegőpontos számot generál *)
...
(* Három karakteres stringből idöstringet generál *)
...
(* Három karakteres stringből egy dátumot dekódol *)
...
begin
  ...
  // Ha már kettő napja van, akkor a régebbit törölni kell:
...
begin
  if _meret = 'KICSI' then
    begin
      Height := 450;
      _meret := 'NAGY';
      MeretGomb.Caption := 'KESKENY KÉPERNYŐ';
    end else
    begin
      if _flipflop then
        begin
          Height := 90;
          _meret := 'KICSI';
          MeretGomb.Caption := 'TELJES KÉPERNYŐ';
          _flipflop := true;
        end;
    end;
  Form1.Repaint;  
