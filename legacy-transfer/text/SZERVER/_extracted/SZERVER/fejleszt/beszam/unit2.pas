...
begin
  ...
  MegnevezesSor(8,'Működést javító ötletek,fejlesztési - vagy költség-takarékossági ötletek');
  ErtektarSor;
  TervezSor;

  _text := 'Iroda – értéktár – területi vezetői együttműködés: A központi irányítás (iroda), az értéktár és a területi vezetők együttműködésével kapcsolatos rövid beszámoló';
  Megnevezessor(9,_text);
  Megnevezessor(0,'az adott hónap vonatkozásában.');
  ErtektarSor;
  TervezSor;

  MegnevezesSor(10,'Főértéktárral való együttműködés: Ebben a pontban szükséges részletezni a főértéktárosokkal kapcsolatos együttműködés adott hónapra vonatkozó rövid értékelését');
  ErtektarSor;
  TervezSor;
...
begin
  ...
  _oxl.worksheets[_korzetindex].cells[_prisor,2] := 'Ügyvezetői hozzáfűzés';
end;
...
begin
  ...
  _oxl.worksheets[_korzetindex].range[_rangestr].Font.Size := 14;
  _oxl.worksheets[_korzetindex].cells[_prisor,2] := 'Területi vezető külön pontjai:';
  inc(_prisor);
  _rangestr := 'A' + inttostr(_prisor)+':M' +inttostr(_prisor);
  _oxl.worksheets[_korzetindex].range[_rangestr].Interior.color := clwhite;
  ...
  Extrafocim;
  _oxl.worksheets[_korzetindex].cells[_prisor,2] := 'Ellenőrzések eredménye';
  ...
  _oxl.worksheets[_korzetindex].range[_rangestr].Font.Size := 12;
  _oxl.worksheets[_korzetindex].range[_rangestr].Font.Bold := False;
  _oxl.worksheets[_korzetindex].cells[_prisor,2] := 'A területi vezető által végzett havi ellenőrzések tapasztalatai, a leggyakrabban előforduló hibák leírásával, valamint a jó gyakorlatok bemutatása.';
  ...
  _oxl.worksheets[_korzetindex].range[_rangestr].Font.Size := 12;
  _oxl.worksheets[_korzetindex].range[_rangestr].Font.Bold := False;
  _oxl.worksheets[_korzetindex].cells[_prisor,2] := 'Minden – nem rendszeresen, a szokásos munkamenet részét képező – eseti feladat végrehajtásának felsorolása (befejezett feladatok esetében) illetve a folyamatban lévő feladatok esetében annak állapota.';
  ...
  _oxl.worksheets[_korzetindex].range[_rangestr].Font.Size := 12;
  _oxl.worksheets[_korzetindex].range[_rangestr].Font.Bold := False;
  _oxl.worksheets[_korzetindex].cells[_prisor,2] := 'A területhez tartozó gépjárművek állapotának rövid bemutatása, különös tekintettel a szervízek, illetve az esetleges sérülésekre.';
  Barnacsik;
...
begin
  ...
  _oxl.worksheets[_korzetindex].range[_rs].MergeCells := True;
  _oxl.worksheets[_korzetindex].cells[_k+8,2] := 'ÜGYVEZETŐI HOZZÁFŰZÉS';
...
begin
  ...
  _oxl.worksheets[_korzetindex].Cells[_p,8] := 'VALUTAVÁLTÁS ELÖZŐ ÉVHEZ KÉPEST';
  _oxl.worksheets[_korzetindex].Cells[_p,12] := inttostr(_kertev)+ ' '+_honev[_kertho]+' HAVI BRUTTÓ HASZON';
  _oxl.worksheets[_korzetindex].Cells[_p,13] := 'BRUTTÓ HASZON ALAKULÁSA ELÖZŐ ÉVHEZ KÉPEST';
  ...
  _oxl.worksheets[_korzetindex].Cells[_p+4,7] := 'WESTERN UNION ALAKULÁSA ELÖZŐ ÉVHEZ KÉPEST';
  _oxl.worksheets[_korzetindex].Cells[_p+4,11] := 'KEZELÉSI DÍJ ELÖZŐ ÉVHEZ KÉPEST';
  _oxl.worksheets[_korzetindex].Cells[_p+4,13] := 'TRANZ ADÓ ALAKULÁSA AZ ELÖZŐ ÉVHEZ KÉPEST';
