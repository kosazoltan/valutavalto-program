...
begin
  KovirPanel.Caption := 'A KOVETKEZŐ:';
  Nextedit.clear;
  with NextPanel do
    begin
      Top := 416;
...
begin
  KovirPanel.Caption := 'A KÖVETKEZŐT:';
  Nextedit.clear;
  with NextPanel do
    begin
      Top := 416;
...
begin
  ...
  FINDparancs(_pcs);
  _ptPtr := 1;
  if _seekType=0 then   // Név eleje az első feltétel;
    begin
      _kernev := _ksz[0];
...
begin
  ...
  if _tolevhos>_igevhos then
    begin
      ShowMessage('Érvénytelen időintervallum !');
      exit;
    end;
  ...
  begin
    ShowMessage('NEM TALÁLTAM BIZONYLATOT A KÉRT IDŐINTERVALLUMBAN');
    with SeekMenuPanel do
      begin
        Top := 240;
        Left := 390;
