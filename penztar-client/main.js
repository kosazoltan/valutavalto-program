...
function getDbPath() {
  ...
  if (!node_fs.default.existsSync(valutaDir))
    try {
...
function resolveWasmPath() {
  ...
  if (electron.app.isPackaged) {
    ...
    candidates.push(
      node_path.default.join(electron.app.getAppPath(), 'resources', 'sql-wasm.wasm'),
    );
    ...
    candidates.push(
      node_path.default.join(process.cwd(), 'node_modules/sql.js/dist/sql-wasm.wasm'),
    );
  }
...
async function initDatabase() {
  try {
    ...
    for (const colDef of ['local_reference_number TEXT', 'idempotency_key TEXT'])
      try {
...
function savePendingConversion(
  fromCurrencyId,
  fromCurrencyCode,
  toCurrencyId,
  toCurrencyCode,
...
function savePendingBankTransaction(
  transactionType,
  currencyCode,
  amount,
  exchangeRate,
...
function saveCachedBranchStatus(
  branchCode,
  branchName,
  companyId,
  lastSyncAt,
  ...
  try {
    const qrDataUrl = await (
      await Promise.resolve().then(() =>
        /* @__PURE__ */ require_chunk.__toESM(require('./lib-C2WqpPNS.js').default),
      )
    ).toDataURL(qrText, {
      width: 120,
      margin: 1,
...
async function printToSerialPrinter(data, serialPort) {
  ...
  try {
    const { printReceiptToSerial } = await Promise.resolve().then(() =>
      require('./serial-printer-SF75wf_9.js'),
    );
    return await printReceiptToSerial(data, { port: serialPort });
  } catch (err) {
    electron_log_main.default.warn(
      '[PRINTER] Soros nyomtató hiba, fallback Electron print-re:',
      err,
    );
    return false;
  }
  ...
  try {
    ...
    return await new Promise((resolve) => {
      printWindow.webContents.print(printOptions, (success, failureReason) => {
        if (!success)
          electron_log_main.default.warn(`[PRINTER] Nyomtatás sikertelen: ${failureReason}`);
        resolve(success);
      });
    });
  } catch (err) {
...
async function printReceipt(data, printerName, serialPort) {
  try {
    electron_log_main.default.info(
      `[PRINTER] Nyomtatás indítása: ${data.type} ${data.receiptNumber}`,
    );
    if (serialPort) {
      if (await printToSerialPrinter(data, serialPort)) {
        electron_log_main.default.info(
          `[PRINTER] Soros blokknyomtató (${serialPort}): OK — ${data.receiptNumber}`,
        );
        return true;
      }
      electron_log_main.default.warn(
        `[PRINTER] Soros port ${serialPort} sikertelen, Electron fallback...`,
      );
    }
    ...
    if (electronSuccess)
      electron_log_main.default.info(`[PRINTER] Electron print: OK — ${data.receiptNumber}`);
    else
      electron_log_main.default.error(
...
var SyncEngine = class {
  ...
  getBootstrapCredentials() {
    ...
    const password =
      ...
      getConfig('bootstrap_password')?.trim() ||
      '';
    const roleCode =
    ...
    try {
      if (electron.safeStorage.isEncryptionAvailable()) {
        setConfig(
          ...
          electron.safeStorage.encryptString(token).toString('base64'),
        );
  ...
  getAuthToken() {
    ...
    if (encryptedToken && electron.safeStorage.isEncryptionAvailable())
      try {
  ...
  async syncTransaction(serverUrl, token, tx) {
    ...
    if (tx.handling_fee !== null && tx.handling_fee !== void 0)
      body['handlingFee'] = tx.handling_fee;
    if (tx.discount_percent !== null && tx.discount_percent !== void 0)
  ...
  async syncConversion(serverUrl, token, conversion) {
    ...
    if (conversion.handling_fee !== null && conversion.handling_fee !== void 0)
    ...
    await httpPost(
      ...
      body,
      token,
      conversion.idempotency_key ?? void 0,
    );
  }
  async syncBankTransaction(serverUrl, token, bankTransaction) {
    ...
    if (
      ...
      bankTransaction.vault_territory_id !== void 0
    )
    ...
    await httpPost(
      ...
      body,
      token,
      bankTransaction.idempotency_key ?? void 0,
    );
  }
  async syncStorno(serverUrl, token, storno) {
    ...
    if (storno.custom_exchange_rate !== null && storno.custom_exchange_rate !== void 0)
  ...
  async syncHandoverOperation(serverUrl, token, operation) {
    if (operation.operation_type === 'GENERATE') {
      await httpPost(
        ...
        token,
        operation.idempotency_key ?? void 0,
      );
      return;
    }
  ...
  async syncDistributions() {
    try {
      ...
      console.warn(
        '[SyncEngine] Distribution sync hiba:',
        err instanceof Error ? err.message : err,
      );
    }
  }
    ...
    try {
      ...
      for (const tx of pending)
        try {
          ...
          if (tx.currency_id !== null && tx.currency_id !== void 0)
  ...
  async cacheBranchStatus() {
    try {
      ...
      console.warn(
        '[SyncEngine] Branch status cache hiba:',
        err instanceof Error ? err.message : err,
      );
    }
  }
  ...
  async syncCashDeskMasterData() {
    try {
      ...
      console.warn(
        '[SyncEngine] Pénztár törzs sync hiba:',
        err instanceof Error ? err.message : err,
      );
    }
  }
  ...
  async syncWorkerMasterData() {
    try {
      ...
      console.warn(
        '[SyncEngine] Dolgozó törzs sync hiba:',
        err instanceof Error ? err.message : err,
      );
    }
  }
...
function registerCameraHandlers() {
  electron.ipcMain.handle(
    ...
    async (_event, transactionId, videoBuffer, extension) => {
  ...
  electron.ipcMain.handle('camera-local-storage-stats', async () => {
    if (!node_fs.default.existsSync(CAMERA_DIR))
      return {
        totalUsageBytes: 0,
...
function registerScannerHandlers() {
  electron.ipcMain.handle(
    'scan-save-document',
    async (_event, transactionId, documentType, imageBase64) => {
  ...
  electron.ipcMain.handle('scan-get-document', async (_event, filepath) => {
    const resolved = node_path.default.resolve(filepath);
    if (!resolved.startsWith(node_path.default.resolve(SCAN_DIR)))
  ...
  electron.ipcMain.handle('scan-list-documents', async (_event, transactionId) => {
    ...
    for (const dateDir of dateDirs) {
      ...
      if (
        !node_fs.default.existsSync(candidate) ||
        !node_fs.default.statSync(candidate).isDirectory()
      )
        continue;
      const files = node_fs.default.readdirSync(candidate);
      for (const file of files)
        if (file.endsWith('.enc')) results.push(node_path.default.join(candidate, file));
    }
    return results;
  });
}
...
function createWindow() {
  ...
  mainWindow.webContents.on('console-message', (_event, level, message, line, sourceId) => {
    if (level >= 2)
      electron_log_main.default.warn(`[Renderer] L${level} ${sourceId}:${line} — ${message}`);
  });
...
electron.ipcMain.handle('print-receipt', async (_event, dataJson) => {
  try {
    return await printReceipt(
      JSON.parse(dataJson),
      getConfig('printer.deviceName') ?? void 0,
      getConfig('printer.serialPort') ?? void 0,
    );
  } catch (err) {
...
electron.ipcMain.handle('list-serial-ports', async () => {
  try {
    const { listSerialPorts } = await Promise.resolve().then(() =>
      require('./serial-printer-SF75wf_9.js'),
    );
...
electron.ipcMain.handle('open-cash-drawer', async () => {
  try {
    ...
    if (!serialPort) return false;
    const { openCashDrawer } = await Promise.resolve().then(() =>
      require('./serial-printer-SF75wf_9.js'),
    );
...
electron.ipcMain.handle(
  ...
  async (_event, targetBranchCode, currencyCode, amount, denominations, note) => {
...
electron.ipcMain.handle(
  ...
  async (_event, sourceBranchCode, currencyCode, amount, note) => {
...
electron.ipcMain.handle('secure-store-token', async (_event, token) => {
  try {
    if (!electron.safeStorage.isEncryptionAvailable()) {
      electron_log_main.default.warn(
        '[SafeStorage] Encryption not available, falling back to config store',
      );
      setConfig('auth_token', token);
      return true;
    }
...
electron.ipcMain.handle('secure-load-token', async () => {
  try {
    ...
    if (plaintext) {
      electron_log_main.default.info(
        '[SafeStorage] Migrating plaintext token to encrypted storage',
      );
      if (electron.safeStorage.isEncryptionAvailable()) {
        setConfig(
          'auth_token_encrypted',
          electron.safeStorage.encryptString(plaintext).toString('base64'),
        );
        deleteConfig('auth_token');
      }
      return plaintext;
...
electron.app.whenReady().then(async () => {
  ...
  electron.protocol.handle('app', (req) => {
    ...
    if (!(node_path.default.extname(filePath) !== ''))
      filePath = node_path.default.join(distPath, 'index.html');
    const resolved = node_path.default.resolve(filePath);
    ...
    if (!resolved.startsWith(resolvedDist + node_path.default.sep) && resolved !== resolvedDist) {
      electron_log_main.default.warn(
        `[Protocol] Path traversal blokkolva: ${req.url} → ${resolved}`,
      );
      filePath = node_path.default.join(distPath, 'index.html');
    }
