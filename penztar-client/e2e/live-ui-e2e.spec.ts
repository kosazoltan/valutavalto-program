...
test.describe('Live UI E2E — Valutaváltó Pénztár', () => {
  test('01 — Login + Dashboard + Tranzakció', async ({ page }) => {
    ...
    await page
      .waitForSelector('text=Oldal betöltése...', { state: 'hidden', timeout: 15000 })
      .catch(() => {
        console.log('[4] Loading spinner nem volt látható vagy már eltűnt');
      });

    await page.screenshot({
      path: path.join(SCREENSHOTS_DIR, '04-transactions-page.png'),
      fullPage: true,
    });
    ...
    if (!currentUrl.includes('/transactions')) {
      await page.goto(BASE_URL + '/transactions/cashier', {
        waitUntil: 'domcontentloaded',
        timeout: 15000,
      });
      await page.waitForLoadState('networkidle', { timeout: 15000 });
    ...
    await page.screenshot({
      path: path.join(SCREENSHOTS_DIR, '05-before-transaction.png'),
      fullPage: true,
    });
    ...
    if (eurCount > 0) {
      ...
      if (await currencySelect.isVisible()) {
        ...
        await page.screenshot({
          path: path.join(SCREENSHOTS_DIR, '05b-eur-selected.png'),
          fullPage: true,
        });
      }
    }
