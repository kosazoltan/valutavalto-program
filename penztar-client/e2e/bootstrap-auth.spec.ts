...
const API_BASE_URL =
  process.env.VITE_API_URL || process.env.API_BASE_URL || 'https://excvaluta.com/api/v1';
...
test.describe('Electron Bootstrap Auth + Database Cache', () => {
  ...
  test('Login - PENZTAR_BOOTSTRAP credentials', async ({ request }) => {
    ...
    console.log(
      `✓ Login successful: ${BOOTSTRAP_WORKER} @ ${BOOTSTRAP_COMPANY}, token length: ${token?.length}`,
    );
  });
  ...
  test('Exchange Rates (Árfolyam Cache) - 27+ currencies', async ({ request }) => {
    ...
    const hasHUFRate = ratesList.some(
      (r: { targetCurrency?: string; sourceCurrency?: string }) =>
  ...
  test('Workers (Pénztáros Törzs) - BORSI exists', async ({ request }) => {
    ...
    const body: WorkerData[] | { content?: WorkerData[]; data?: WorkerData[] } =
      await response.json();
    ...
    const borsi = workersList.find(
      (w: WorkerData) => w.workerCode === 'BORSI' || w.code === 'BORSI',
    );
  ...
  test('Branches (Pénztár/Iroda Törzs) - TISZA exists', async ({ request }) => {
    ...
    const body: BranchData[] | { content?: BranchData[]; data?: BranchData[] } =
      await response.json();
