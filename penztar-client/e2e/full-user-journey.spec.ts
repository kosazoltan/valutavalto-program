...
test.describe('Full User Journey — Emberi viselkedés szimuláció', () => {
  ...
  test('4. Valuta VÉTEL — új tranzakció oldal', async () => {
    ...
    const formExists = await page.locator('form, [class*="transaction"], [class*="Transaction"]').first().isVisible().catch(() => false)
    ...
    const amountInputs = await page.locator('input[type="number"], input[type="text"][inputmode="decimal"]').count()
    console.log(`✓ Amount inputs found: ${amountInputs}`)
    
    // Try filling a basic transaction (won't submit - just check form works)
    const currencyInput = page.locator('input[placeholder*="Valuta"], input[name*="currency"], [class*="currency"]').first()
  ...
  test('6. Ügyfél keresés', async () => {
    ...
    // Search input
    const searchInput = page.locator('input[placeholder*="Keresés"], input[placeholder*="Search"], input[type="search"]').first()
    ...
    // New customer button
    const newBtn = page.locator('button:has-text("Új"), button:has-text("New"), a[href*="/customers/new"]').first()
  ...
  test('6b. Új ügyfél form betöltődik', async () => {
    ...
    // Check form fields
    const nameInput = page.locator('input[name*="name"], input[name*="Name"], input[placeholder*="Név"]').first()
  ...
  test('7. Árfolyamok oldal — árak megjelennek', async () => {
    ...
    const hasEUR = await page.locator('text=/EUR/').first().isVisible().catch(() => false)
    const hasUSD = await page.locator('text=/USD/').first().isVisible().catch(() => false)
  ...
  test('16. Pénztáros főmenü', async () => {
    ...
    const quickActions = await page.locator('button, a').filter({ hasText: /Vétel|Eladás|Átváltás|Nyitás|Zárás/i }).count()
  ...
  test('20. Kijelentkezés', async () => {
    ...
    const logoutBtn = page.locator('button:has-text("Kijelentkezés"), button:has-text("Logout"), [class*="logout"]').first()
