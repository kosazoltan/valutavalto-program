...
test('02 — EUR VÉTEL tranzakció', async ({ page }) => {
  ...
  if (buyBtnVisible) {
    ...
    const buyAlt = page
      .locator('button:has-text("VÉTEL"), button:has-text("Vétel"), button:has-text("BUY")')
      .first();
  ...
  if (saveBtnVisible) {
    ...
    const saveAlt = page
      .locator(
        'button:has-text("Mentés"), button:has-text("Mentés és nyomtatás"), button:has-text("Kaydet")',
      )
      .first();
  ...
  const toastEl = page
    .locator('[role="alert"], .toast, .Toastify, [class*="toast"], [class*="notification"]')
    .first();
...
test('03 — EUR ELADÁS tranzakció', async ({ page }) => {
  ...
  if (await sellBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    const sellAlt = page
      .locator('button:has-text("ELADÁS"), button:has-text("Eladás"), button:has-text("SELL")')
      .first();
  ...
  if (await saveBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    const saveAlt = page
      .locator('button:has-text("Mentés"), button:has-text("Mentés és nyomtatás")')
      .first();
    if (await saveAlt.isVisible({ timeout: 3000 }).catch(() => false)) {
      await saveAlt.click();
    }
...
test('04 — Tranzakciólista', async ({ page }) => {
  ...
  const rows = page.locator(
    'table tbody tr, [class*="transaction-row"], [class*="TransactionRow"]',
  );
...
test('05 — Átadás-átvétel (Transfers)', async ({ page }) => {
  ...
  const tabs = page.locator(
    '[role="tab"], button:has-text("Kimenő"), button:has-text("Bejövő"), button:has-text("Függőben")',
  );
  const tabCount = await tabs.count();
  console.log(`[TRANSFERS] Tabok száma: ${tabCount}`);

  for (let i = 0; i < Math.min(tabCount, 3); i++) {
    const tabText = await tabs
      .nth(i)
      .textContent()
      .catch(() => '');
  ...
  const newTransferBtn = page
    .locator(
      'button:has-text("Új átadás"), button:has-text("Átadás"), [data-testid*="new-transfer"]',
    )
    .first();
  ...
  for (const route of routes) {
    ...
    try {
      ...
      const is404 = await page
        .locator('text=404, text=Nem található, text=Not Found')
        .first()
