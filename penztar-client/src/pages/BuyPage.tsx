...
export default function BuyPage() {
  ...
  const [feePercent] = useState(DEFAULT_FEE_PERCENT);
  ...
  const buyRate = selectedCurrency ? getBuyRate(selectedCurrency) : 0;
  const unit = currencyInfo?.unit ?? 1;

  // Automatikus számítás (vásárlás: mi fizetünk HUF-ot a devizáért)
  useEffect(() => {
    if (!selectedCurrency || buyRate === 0) return;

    if (inputMode === 'foreign') {
      const foreign = parseFloat(foreignAmount);
      if (!isNaN(foreign) && foreign > 0) {
        const huf = (foreign / unit) * buyRate;
      ...
      if (!isNaN(huf) && huf > 0) {
        const foreign = (huf / buyRate) * unit;
        setForeignAmount(foreign.toFixed(2));
      } else {
        setForeignAmount('');
      }
    }
  }, [foreignAmount, hufAmount, inputMode, buyRate, selectedCurrency, unit]);

  const hufNumeric = parseFloat(hufAmount) || 0;
  const feeAmount = Math.round(hufNumeric * (feePercent / 100));
  const totalHuf = hufNumeric - feeAmount;
  const roundedHuf = roundHuf(totalHuf);
  const roundingDiff = roundedHuf - totalHuf;
  ...
  const handleSanctionScreen = useCallback(async (customerName: string, docNumber?: string) => {
    ...
    try {
      ...
      if (result.riskLevel === 'CLEAR') {
        console.log('[BuyPage] Szankciós szűrés: CLEAR — nincs találat');
      } else {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (isOnline) {
        ...
        setSuccess(
          `✅ Vásárlás rögzítve — ${parseFloat(foreignAmount)} ${selectedCurrency} = ${roundedHuf.toLocaleString('hu-HU')} Ft (offline — szinkronizálódik később)`,
        );
      }
    ...
    <div className="flex h-screen flex-col">
      <header className={`${headerColor} px-6 py-3 text-white`}>
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold">🛒 Deviza Vásárlás</h1>
          <button
      ...
      <main className="flex-1 overflow-auto p-6">
        <div className="mx-auto grid max-w-6xl grid-cols-3 gap-6">
          ...
          {/* KÖZÉP — Összeg bevitel */}
          <div className="col-span-1 space-y-4">
            {selectedCurrency && (
              <div className="rounded-xl border bg-green-50 p-4">
                <p className="text-sm text-gray-500">Vételi árfolyam</p>
                <p className="text-3xl font-bold text-green-700">
                  {buyRate.toFixed(2)} Ft
                  {unit > 1 && (
              ...
              <div className="rounded-xl border bg-green-50 p-4 space-y-1">
                ...
                {feeAmount > 0 && (
                  <div className="flex justify-between text-sm text-orange-600">
                    <span>Kezelési díj ({feePercent}%):</span>
                    <span className="font-mono">
                      -{feeAmount.toLocaleString('hu-HU')} Ft
                    </span>
                  </div>
                )}
                <div className="flex justify-between text-lg font-bold text-green-700">
                  <span>Kifizetendő:</span>
          ...
          <div className="col-span-1 space-y-4">
            ...
            >
              {isSubmitting
                ? '⏳ Feldolgozás...'
                : `🛒 Vásárlás véglegesítése — ${roundedHuf.toLocaleString('hu-HU')} Ft`}
            </button>
          </div>
        </div>
      </main>
