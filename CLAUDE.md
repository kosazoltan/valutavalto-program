...
- **Verzió:** **v2.5.86** (2026-05-20 — capability-features [#718–#720] + átadás-átvétel csomag [#721,#723,#724] + Főlap A-oszlop [#722]).
  - **v2.5.86 (admin-merged `74e9e6cbe`):** PR #724 — Több valuta egy átadólapon, **backend-alap (DORMANT)**: V243 `transfer_lines` tábla (FK `currency(id)` — KRITIKUS Copilot-fix, eredetileg `currencies` volt → outage-veszély elhárítva), `TransferLine` entity + `Transfer.lines`, CreateTransferDto/TransferDto.lines, TransferService soronkénti könyvelés (effectiveLines, FF/UF/receive per-line, dup-valuta validáció), 6 teszt. ⚠️ HÁTRALÉVŐ #6: **frontend multi-row UI** (ekkor lesz éles) → Electron lines-perzisztencia.
