...
- **Verzió: v2.27.74** (2026-06-01). Production **HEALTHY 200**. Friss munka: koncurrencia + security mérföldkő (#944–#954): napi sztornó-plafon (prior-day NEVER + aznapi max-3) + **globális cash-balance lock-ordering deadlock-megelőzés** (CashLockOrdering — single-branch növekvő currencyId + cross-branch (branch,currency)-tuple + cash↔receipt_sequence cash-first egységesítés MINDEN pénzmozgató úton: Buy/Sell/Reversal/PartialRefund/Conversion/MultiLine/Trade/Transfer/Reservation) + CodeQL log-injection (CWE-117) fix + sztornó 4-szem-elv konfigurálható flag (default OFF). Teljes release-történet: [vault/sessions/release-history-archive-through-2.27.25.md](vault/sessions/release-history-archive-through-2.27.25.md) + [vault/sessions/2026-05-31-reversal-cap-concurrency-security-milestone.md](vault/sessions/2026-05-31-reversal-cap-concurrency-security-milestone.md).
- Folyó: a korábbi architect-mode audit follow-upok többsége LEZÁRVA (AML reverseAccumulation → #873 v2.27.28; PoliceRequest multi-tenant → V270 migráció + `companyId`). Nyitott/homályos: Electron HA failover sync-URL (kliens Scaleway-standby fallback — spec-tisztázás szükséges). Telepítő: v2.27.70 **UNSIGNED candidate (NotSigned/not-sendable)** a DigiCert EV CS cert kiadásáig.
