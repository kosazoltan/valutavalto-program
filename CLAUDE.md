...
- **Nyitott következő feladatok (2026-04-22 ota, friss session utan):**
  - **P0 (éles pénztár frissítés):** user v2.1.7 reinstall az éles gépen — 1) `Penztar-Eltavolito-2.1.7-20260421.exe` admin joggal, 2) `Penztar-Setup-2.1.7-20260421.exe` admin joggal, 3) SetupWizard 5 lépés (Iroda → Program típus → Szerver (**Kapcsolat tesztelése** gomb kötelező!) → Admin jelszó → Telepítés), 4) belépés a bootstrap admin credentials-szel (ld. setup wizard SetupWizard 4. lepes + 1Password/secure vault), 5) új VÉTEL teszt → **`V<branchCode>000001`** NGM-bizonylat (pl. `V035000001` a BR035 branch első VÉTELére — **NEM** `VEBC000001`; a prefix a `receipt_sequence` worker branch-kódjából jön, nem a company-ból). 2026-04-22 teszt igazolta: `V035000004` generálódik a BR017 branch + dev ADMIN userre.
  - **P0 (cash_balance deployment gap):** Issue #110 — a `CashBalanceService.initializeBranchBalances()` létezik, de nincs publikus endpoint / automatikus hívó. Eredmény: új branch-eken a tranzakció sync `404 Kassza egyenleg nem található` hibával elesik. Workaround: SSH Hetzner primary + SQL INSERT minden aktív currency-re. Javaslat: `POST /api/v1/cash-balances/init-branch/{branchId}` admin endpoint + auto-init session open-kor.
