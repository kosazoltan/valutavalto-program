...
type
  ...
  NonQuitGomb: TBitBtn;
    ...
    function GetCegNev(_etsz: string): string;
...
var
  ...
  _homepenztarnev,_etarnev,_cegnev: string;
  _code: integer;
...
function backuprestore:integer;stdcall;external 'c:\ertektar\bin\mentes.dll' name 'backuprestore';
...
begin
  ...
  with ValQuery do
     begin
       ...
       _homepenztarnev := uppercase(trim(FieldByName('PENZTARNEV').asstring));
       Close;
     end;
  ...
  _homepenztarnev := _homepenztarnev+' ÉRTÉKTÁR';
  _cegnev := getcegnev(_homepenztarkod);
...
begin
  ...
  _mondat := 'az ' + _cegnev;
  KozepreIr(_mondat);
  WriteLn(_LFile,_homepenztarnev + ' dolgozoja  ki-');
...
function TQuitForm.GetCegNev(_etsz: string): string;

begin
  result := 'EXCLUSIVE BEST CHANGE ZRT';
end;
