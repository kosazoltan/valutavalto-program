...
begin
   ...
   if (_tipus='F') or (_tipus='U') then
     begin
       _ftipus := midstr(_bizonylatszam,2,1);
       if _ftipus='F' then _forintszamla := True;
     end;  
...
begin
  ...
  if _kftnev='BEST' then _aktadoszam := '11788670-2-41';
  if _kftnev='EAST' then _aktadoszam := '23008975-2-41';
  if _kftnev='PANNON' then _aktadoszam := '23009017-1-41';
  _cegnev      := 'EXCLUSIVE ' +_kftnev + ' CHANGE KFT.';
end;
...
begin
  ...
  StartNyomtatas;
  if _forintszamla then TextKiiro;
end;
...
begin
  ...
  writeLn(_LFile,'      Bizonylatszam: ' + _bizonylatszam);
  writeLn(_LFile,'    Stornozas kelte: '+_datum);
  WriteLn(_LFile,'  Stornozott osszeg: '+ftform(_bankjegy,_aktdnem));
