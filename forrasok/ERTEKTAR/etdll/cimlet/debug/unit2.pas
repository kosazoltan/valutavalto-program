...
type
  TCimletezes = class(TForm)
    ...
    function Gyomlal(_s: string): string;
...
begin
  ...
  for i := 1 to 14 do
    begin
      if _cimflag[i] then
        begin
          ...
          _rr[i].Caption := Ftform(_aktrr,11);
          _sumctrl := _sumdevertek+_aktrr;
          if _sumctrl<0  then
            begin
              Showmessage('TÚL MAGAS A KÉSZLETÉRTÉK');
              _rr[i].Caption := '';
              _ed[i].text := '0';
              exit;
            end;
   ...
   SftPanel.Caption := Ftform(_sumDevertek,11);
...
begin
  ...
  if _hh=5 then
    begin
      if _num<1000 then exit;
      while length(result)<5 do result := ' ' + result;
      result := leftstr(result,2)+' '+midstr(result,3,3);
      exit;
    end;
...
begin
  ...
  _akttext := gyomlal(_aktedit.text);
  val(_akttext,_aktbankjegy,_code);
  if _code<>0 then _aktbankjegy := 0;

  _aktedit.text := ftform(_aktbankjegy,5);
...
begin
  ...
  while not Cquery.eof do
    begin
      ...
      _pcs := 'INSERT INTO CIMLETEK (DATUM,VALUTANEM,BANKJEGY,';
...
function Tcimletezes.Gyomlal(_s: string): string;

var _asc,_sw,_z: byte;
...
begin
  ...
  while _z<=_sw do
    begin
      _asc := ord(_s[_z]);
      if (_asc>47) and (_asc<58) then result := result + chr(_asc);
      inc(_z);
    end;
end;
