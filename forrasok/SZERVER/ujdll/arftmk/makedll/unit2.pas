...
type
  TARFOLYAMTMK = class(TForm)
    ...
    ARFOLYAMEDIT: TEdit;
    Shape3: TShape;
    ARFOLYAMTABLAVETELIARFOLYAM: TFloatField;
    ARFOLYAMTABLAELADASIARFOLYAM: TFloatField;
    ARFOLYAMTABLAMNBARFOLYAM: TIntegerField;
    ...
    procedure MNBArfKikuldo(Sender: TObject);
    procedure OldArfolyamDelete;
    procedure OldArfRemove;
    procedure UjKikuldo;
    ...
    procedure ARFOLYAMEDITKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
var
  ...
  _nevlen,_elszamolasiarfolyam,_mnbarfolyam,_code: integer;

  _valutadarab: byte;
  _betu       : char;
  _top,_left,_height,_width,_aktev,_aktho,_aktnap: word;

  _mamas,_aktdnem,_aktdnev,_pcs,_kit,_mnbs: string;
...
begin
  ...
  ArfolyamTabla.IndexFieldNames := 'VALUTANEM';
...
// ==============================================================
         procedure TArfolyamTmk.MNBarfKikuldo(sender: TObject);
// ==============================================================
...
begin
  ...
  while not ArfolyamTabla.Eof do
    begin
      _aktdnem := ArfolyamTabla.FieldByName('VALUTANEM').asString;
      _mnbarfolyam:= ArfolyamTabla.FieldByName('MNBARFOLYAM').asInteger;

      DnemKod(_aktdnem);
      blockwrite(_iras,_bytetomb,2);
      _intar := 10000*_mnbarfolyam;
      Integkod(_intar);
      Blockwrite(_iras,_byteTomb,5);
      ArfolyamTabla.Next;
    end;
...
procedure TARFOLYAMTMK.ARFOLYAMRACSDblClick(Sender: TObject);


begin

  _aktdNem := ArfolyamTabla.fieldbyname('VALUTANEM').asString;
  if (_aktdnem='HUF') or (_aktdnem='EUA') then
    begin
      Arfolyamtabla.next;
      exit;
    end;

  _aktdnEV := trim(ArfolyamTabla.fieldbyname('VALUTANEV').asString);
  _MNBarfolyam := arfolyamTabla.fieldByName('MNBARFOLYAM').asInteger;

  ValnevPanel.Caption := _aktdnev;
  Arfolyamedit.text := inttostr(_elszamolasiarfolyam);
...
procedure TARFOLYAMTMK.ARFOLYAMEDITKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if ord(key)<>13 then exit;
  _mnbs := trim(Arfolyamedit.Text);
  if _MNBs='' then exit;
  val(_mnbs,_mnbarfolyam,_code);
  if _code<>0 then exit;
  Arfolyamtabla.edit;
  Arfolyamtabla.FieldByName('MNBARFOLYAM').asInteger := _mnbarfolyam;
  Arfolyamtabla.post;
  Beiropanel.visible := False;

end;

// ==============================================================
                  procedure TArfolyamTmk.UjKikuldo;
// ==============================================================
...
begin
  ...
  _aPath := 'c:\receptor\mail\exchange\arf' + midstr(_mamas,6,2)+midstr(_mamas,9,2)+'.DAT';
  OldArfRemove;

  AssignFile(_iras,_aPath);
  Rewrite(_iras);
  ...
  while not ArfolyamTabla.Eof do
    begin
      ...
      _hi := trunc(_mnbArfolyam/256);
      _lo := _mnbArfolyam-trunc(256*_hi);
      _bytetomb[1] := _lo;
      _bytetomb[2] := _hi;
      blockwrite(_iras,_bytetomb,2);
...
// =============================================================================
                  procedure Tarfolyamtmk.OldArfRemove;
// =============================================================================
...
begin
  _minta := 'c:\receptor\mail\exchange\arf*.dat';
  _cc := 0;
  if FindFirst(_minta, faAnyFile, _srec) = 0 then
    begin
      repeat
  ...
  if _cc>0 then
    begin
      ...
      while _y<=_cc do
        begin
          _dfile := _delfile[_y];
          _delpath := 'c:\receptor\mail\exchange\' + _dfile;
          Sysutils.DeleteFile(_delpath);
          inc(_y);
        end;
    end;
