...
begin
  ...
  while _pt<=150 do
    begin
      if (_pt=10) or (_pt=20) or (_pt=40) or (_pt=50) then
        begin
          inc(_pt);
          continue;
        end;

      if (_pt=63) or (_pt=85) or (_pt=120) or (_pt=145) then
        begin
          inc(_pt);
          continue;
        end;
...
begin
  ...
  _apcs := 'INSERT INTO DATA1710 (NEV,PENZTAR,CEGBETU,DATUM,VALNEM1,BANKJEGY1,';
  ...
  _apcs := _apcs + 'FOTEVEKENYSEG,KEPVISNEV,ANYJANEVE,MEGBIZOTTNEV,KEPVISBEO,UGYFELTIPUS)'+_sorveg;
  ...
  _apcs := _apcs + chr(39) + _ugyfeltipus + chr(39) + ')';
...
begin
  ...
  VDbase.close;
  VDbase.DatabaseName := 'c:\receptor\database\receptor.fdb';
  Vdbase.Connected := true;
  with Vquery do
    begin
      Close;
  ...
  while not Vquery.Eof do
    begin
      _uzlet := Vquery.FieldByNAme('UZLET').asInteger;
      _cb[_uzlet] := Vquery.FieldByNAme('CEGBETU').ASSTRING;
      vquery.next;
    end;
  Vquery.close;
  vdbase.close;
