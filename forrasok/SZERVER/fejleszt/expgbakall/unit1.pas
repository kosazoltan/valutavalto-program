...
begin
  ...
  while _tt<=_tabladb do
    begin
      ...
      _source := 'c:\cartcash\1\' + _akttnev;
      _target := 'c:\cartcash\1\' + _trunk + '.dbk';
      _pcs := 'c:\cartcash\1\gbak.exe -v -t -user SYSDBA -password dek@nySo '+_source+' '+_target;
  ...
  while _tt<=_tabladb do
    begin
      _akttnev := _tn[_tt];
      _aktpath := 'c:\cartcash\1\'+_akttnev;
  ...
  while _tt<=_tabladb do
    begin
      ...
      _len := length(_akttnev);
      _trunk := leftstr(_akttnev,_len-4);
      _target := 'c:\cartcash\1\' + _akttnev;
      _source := 'c:\cartcash\1\' + _trunk + '.dbk';
      _pcs := 'c:\cartcash\1\gbak.exe -c -v -user SYSDBA -password dek@nySo '+_source+' '+_target;
  ...
  while _tt<=_tabladb do
    begin
      ...
      _len := length(_akttnev);
      _trunk := leftstr(_akttnev,_len-4);
      _aktpath := 'c:\cartcash\1\'+_trunk+'.dbk';
