...
uses
  ...
  Dialogs, ExtCtrls, StdCtrls, strutils, Buttons, unit1, IBDatabase, DB,
  IBQuery, IBCustomDataSet, IBTable, ComCtrls;

type
  TADATGYUJTES = class(TForm)
    vaszon: TPaintBox;
    Panel1: TPanel;
    FOCIMPANEL: TPanel;
    INDITO: TTimer;
    ...
    procedure Megyevillantas(_megye: byte);
    procedure KorzetAdatLetoltes;
    procedure Forgalomlegyujtes;
    procedure Wulegyujtes;
    procedure Haszonletoltes;
    procedure TranzadoLegyujtes;
    procedure MakeBeszTabla;
    procedure BeszParancs(_ukaz: string);
    procedure AkthaviAdatFeliras;
    procedure ElozoeviAdatLoad;
    procedure TombbolAktvari(_p: byte);
...
var
  ADATGYUJTES: TADATGYUJTES;
  _meg: array[0..18,0..60] of TPoint;
  _akttomb: array[0..60] of TPoint;
  _z,_CC: integer;


  _korzet,_ptss,_aktKorzetPtDb,_aktpenztar: word;
  _masmegye: array[1..10] of byte = (3,5,6,8,10,11,12,16,17,18);
  _korzetsor: array[1..8] of byte = (15,4,0,7,14,2,1,13);
  ...
  _szurke: array[1..10] of tcolor = ($606060,$686868,$707070,$787878,$808080,
                       $888888,$909090,$989898,$A0A0A0,$A8A8A8);
...
// =============================================================================
              procedure TADATGYUJTES.FormActivate(Sender: TObject);
// =============================================================================
...
begin
  Top  := _top + 40;
  Left := _left + 20;
  FocimPanel.Caption := inttostr(_kertev)+' '+_honev[_kertho]+' HAVI BESZÁMOLÓ';
  FocimPanel.Repaint;
...
// =============================================================================
                       procedure TADATGYUJTES.Allcounty;
// =============================================================================
...
// =============================================================================
              procedure TADATGYUJTES.tombtotomb(_st: byte);
// =============================================================================
...
// =========================================================================
                          procedure TADATGYUJTES.Settombok;
// =========================================================================
...
// =============================================================================
  procedure TADATGYUJTES.FillPolygon(ACanvas: TCanvas; APoints: array of TPoint);
// =============================================================================
...
// =============================================================================
             procedure TADATGYUJTES.INDITOTimer(Sender: TObject);
// =============================================================================
...
begin
  ...
  WHILE _Korzet<=8 do
    begin
      Panel1.Caption := _korzetnevek[_korzet]+' KÖRZET';
      Panel1.repaint;
      Csik.Position := _korzet;
      KorzetAdatLetoltes;
      inc(_korzet);
    end;
  ...
  Panel1.caption := 'ADATGYÜJTÉS KÉSZEN VAN';
  panel1.Repaint;
  Modalresult := 1;
end;

// =============================================================================
                  procedure TAdatGyujtes.KorzetAdatLetoltes;
// =============================================================================
...
begin
  ...
  _aktKorzetPtdb := _korzetPtDb[_korzet];
  Rekordcsik.Max := _aktkorzetPtdb;
  _ptss := 1;

  _countySS := _korzetsor[_korzet];
  while _ptss<=_aktkorzetPtDb do
    begin
      rekordCsik.Position := _ptss;
       _aktpenztar := _korzetPtszamSor[_korzet,_ptss];
      ...
      _aktertektar   := _ptertektar[_aktpenztar];
...
// §§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§
// =============================================================================
                      procedure TAdatgyujtes.Forgalomlegyujtes;
// =============================================================================
...
begin
  ...
  Vtabla.close;
  VTabla.TableName := _bftablanev;

  // Ha nem volt ebben a hónapban váltás - kilép
  ...
  // Megnyitjuk a Blokfejeket (VEK tipusokra, storno=1)
  ...
  // HA nincs a hónapban egyetlen rekord sem - akkor kilép
  ...
  while not VQuery.Eof do
   ...
   //   Rekordcsik.Position := _cc;
      ...
      _kezelesidij   := abs(Vquery.FieldByName('KEZELESIDIJ').asInteger);

      _aktkezdij := _aktkezdij + _kezelesidij;

      if _tipus='V' then
        begin
          inc(_aktvugyfel);
          _aktvetel  := _aktvetel + _netto;
        end;

      if _tipus='E' then
        begin
          inc(_akteugyfel);
          _akteladas := _akteladas + _netto;
        end;
      ...
      if _tipus='K' then
        begin
          ...
          _aktvetel  := _aktvetel + _sFt;
          _akteladas := _akteladas + _sFt;
        end;
      Vquery.next;
    end;
  Vquery.close;
  ...
  _vetel[_aktpenztar]   := _aktvetel;
  _eladas[_aktpenztar]  := _akteladas;
  _vugyfel[_aktpenztar] := _aktvugyfel;
  _eugyfel[_aktpenztar] := _akteugyfel;
  _kezdij[_aktpenztar]  := _aktkezdij;
end;


// =============================================================================
                              procedure TAdatgyujtes.Wulegyujtes;
// =============================================================================

var _valutanem: string;
    _bankjegy,_recno2: integer;
...
begin
  ...
  while not vquery.eof do
    begin
      ...
      if _valutanem='USD' then _aktwuusd := _aktwuusd + _bankjegy;
      if _valutanem='HUF' then _aktwuHUF := _aktwuHUF + _bankjegy;
  ...
  _wuUgyfel[_aktpenztar] := _aktwuugyfel;

  _wuusd[_aktpenztar] := _aktwuusd;
  _wuhuf[_aktpenztar] := _aktwuhuf;
end;


// =============================================================================
           procedure TadatGyujtes.MegyeVillantas(_megye: byte);
// =============================================================================

begin
  tombtotomb(_megye);
  Vaszon.Canvas.Brush.Color := clWhite;
  Vaszon.Canvas.Polygon(_akttomb);
  if _megye=0 then
    begin
      tombtotomb(9);
...
// =============================================================================
                 procedure TadatGyujtes.TranzadoLegyujtes;
// =============================================================================
...
begin
  for i := 1 to 150 do _tranzado[i] := 0;
  ...
  _pcs := 'SELECT * FROM TRANZDIJ'+_farok;
  with Tranzquery do
    begin
      Close;
      sql.clear;
  ...
  while not Tranzquery.Eof do
    begin
      _pt := TranzQuery.FieldByName('CASHIER').asInteger;
      _td := TranzQuery.FieldByName('TRANSFEE').asInteger;

      _tranzado[_pt] := _tranzado[_pt] + _td;
      Tranzquery.next;
    end;
  Tranzquery.Close;
  Tranzdbase.close;
  ...
  while _y<=150 do
    begin
      _akttranzado := _tranzado[_y];
      if _akttranzado>0 then
        begin
          _pcs := 'UPDATE ' + _besztablanev + ' SET TRANZADO='+floattostr(_akttranzado)+_sorveg;
          _pcs := _pcs + 'WHERE PENZTAR=' + inttostr(_y);
          BeszParancs(_pcs);
        end;
      inc(_y);
    end;
...
// =============================================================================
                 procedure TAdatgyujtes.MakeBeszTabla;
// =============================================================================
...
begin
  ...
  Besztabla.close;
  besztabla.TableName := _besztablanev;
  ...
  _pcs := 'CREATE TABLE '+_besztablanev+' (' + _sorveg;
...
// =============================================================================
              procedure Tadatgyujtes.BeszParancs(_ukaz: string);
// =============================================================================
...
// =============================================================================
                 procedure Tadatgyujtes.AkthaviAdatFeliras;
// =============================================================================

begin
  TombbolAktvari(_aktpenztar);
  ...
  if _recno>0 then
    begin
      _pcs := 'UPDATE ' + _besztablanev + ' SET ERTEKTAR='+inttostr(_aktertektar)+',';
      _pcs := _pcs + 'PENZTARNEV=' + chr(39)+  _aktpenztarnev + chr(39) + ',';
      _pcs := _pcs + 'VUGYFEL='+ inttostr(_aktvugyfel) + ',';
      _pcs := _pcs + 'VETEL=' + floattostr(_aktvetel) + ',';
      _pcs := _pcs + 'EUGYFEL=' + inttostr(_akteugyfel) + ',';
      _pcs := _pcs + 'ELADAS=' + floattostr(_akteladas) + ',';
      _pcs := _pcs + 'WUUGYFEL=' + inttostr(_aktwuugyfel) + ',';
      _pcs := _pcs + 'WUHUF=' + floattostr(_aktwuhuf) + ',';
      _pcs := _pcs + 'WUUSD=' + floattostr(_aktwuusd) + ',';
      _pcs := _pcs + 'KEZELESIDIJ=' + floattostr(_aktkezdij)+_sorveg;
      _pcs := _pcs + 'WHERE PENZTAR=' + inttostr(_aktpenztar);
    end else
    begin
      _pcs := 'INSERT INTO '+_besztablanev + ' (PENZTAR,ERTEKTAR,PENZTARNEV,VUGYFEL,';
      _pcs := _pcs + 'VETEL,EUGYFEL,ELADAS,WUUGYFEL,WUHUF,WUUSD,KEZELESIDIJ)'+_sorveg;

      _pcs := _pcs + 'VALUES (' + inttostr(_aktpenztar)+',';
      _pcs := _pcs + inttostr(_aktertektar)+',';
      _pcs := _pcs + chr(39) + _aktpenztarnev + chr(39) + ',';
      _pcs := _pcs + inttostr(_aktvugyfel) + ',';
      _pcs := _pcs + floattostr(_aktvetel) + ',';
      _pcs := _pcs + inttostr(_akteugyfel) + ',';
      _pcs := _pcs + floattostr(_akteladas) + ',';
      _pcs := _pcs + inttostr(_aktwuugyfel) + ',';
      _pcs := _pcs + floattostr(_aktwuhuf) + ',';
      _pcs := _pcs + floattostr(_aktwuusd) + ',';
      _pcs := _Pcs + floattostr(_aktkezdij) + ')';
    end;
  BeszParancs(_pcs);
end;

procedure Tadatgyujtes.TombbolAktvari(_p: byte);

begin
  _aktvugyfel := _vugyfel[_p];
  _aktvetel   := _vetel[_p];
  _akteugyfel := _eugyfel[_p];
  _akteladas  := _eladas[_p];
  _aktwuugyfel:= _wuugyfel[_p];
  _aktwuhuf   := _wuhuf[_p];
  _aktwuusd   := _wuusd[_p];
  _aktkezdij  := _kezdij[_p];
end;


// =============================================================================
                 procedure Tadatgyujtes.ElozoeviAdatLoad;
// =============================================================================

begin
  _eevFarok := inttostr(_kertev-2001)+Form1.nulele(_kertho);
  _eevbesztablanev := 'BESZ' + _eevfarok;


  Beszdbase.Connected := true;
  Besztabla.close;
  Besztabla.TableName := _eevbesztablanev;

  if not Besztabla.Exists then
    begin
      Beszdbase.close;
      exit;
    end;
  _pcs := 'SELECT * FROM ' + _eevBeszTablaNev+_sorveg;
  _pcs := _pcs + 'WHERE PENZTAR=' + inttostr(_aktpenztar);
  with Beszquery do
    begin
      Close;
  ...
  while not beszQuery.eof do
    begin
      with beszQuery do
        begin
          _aktvetel    := FieldBynAME('VETEL').asFloat;
          _akteladas   := FieldByName('ELADAS').asFloat;
          _aktvugyfel  := FieldByName('VUGYFEL').asInteger;
          _akteugyfel  := FieldByName('EUGYFEL').AsInteger;
          _aktkezdij   := FieldByName('KEZELESIDIJ').asFloat;
          _aktwuugyfel := FieldbyName('WUUGYFEL').asInteger;
          _aktwuhuf    := FieldByName('WUHUF').asFloat;
          _aktwuusd    := FieldByName('WUUSD').asFloat;
          _akthaszon   := FieldByName('HASZON').asFloat;
          _akttranzado := FieldByName('TRANZADO').asFloat;
         // _akttpenztar := FieldByName('PENZTAR').asInteger;
        end;

      _eevvetel[_aktpenztar]   := _aktvetel;
      _eeveladas[_aktpenztar]  := _akteladas;
      _eevvugyfel[_aktpenztar] := _aktvugyfel;
      _eeveugyfel[_aktpenztar] := _akteugyfel;
      _eevkezdij[_aktpenztar]  := _aktkezdij;
      _eevwuugyfel[_aktpenztar]:= _aktwuugyfel;
      _eevwuhuf[_aktpenztar]   := _aktwuhuf;
      _eevwuusd[_aktpenztar]   := _aktwuusd;
      _eevhaszon[_aktpenztar]  := _akthaszon;
      _eevtranzado[_aktpenztar]:= _aktTranzado;
...
// =============================================================================
                       procedure TadatGyujtes.AdatNullazas;
// =============================================================================
...
procedure TadatGyujtes.Haszonletoltes;
...
begin
  ...
  while not beszQuery.eof do
    begin
      ...
      _aktpenztar := beszQuery.FieldByName('PENZTAR').asInteger;

      _haszon[_aktpenztar] := _akthaszon;
      BeszQuery.next;
    end;
  Beszquery.close;
  Beszdbase.close;
