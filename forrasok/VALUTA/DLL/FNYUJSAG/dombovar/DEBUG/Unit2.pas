...
begin
  ...
  _displayDarab := 1;
  setlength(_doubledisplayPenztar,_displayDarab);

  _doubleDisplayPenztar[0] := 121;

  _height := Screen.Monitors[0].Height;
  _width  := Screen.Monitors[0].width;
  _top    := round((_height-110)/2);
  ...
  with ibQuery do
    begin
      ...
      Sql.Add(_pcs);
      Open;
     _sebesseg := FieldByName('FUTOFENY').asInteger;
     Close;
   end;
 ...
 if _sebesseg<0 then Kilepo.enabled := true
...
// =============================================================================
              procedure TFnyUjsag.BitBtn1Click(Sender: TObject);
// =============================================================================
...
begin
  ...
  if not OpenComport then
    begin
      ShowMessage('Nem sikerült megnyitni a COM1 portot !');
  ...
  if not SetupComport then
    begin
      CloseComport;
      ShowMessage('Nem sikerült beállítani a COM1 portot !');
      Application.Terminate;
      Exit;
    end;
  ...
  if not OpenComport2 then
    begin
      ShowMessage('Nem sikerült megnyitni a COM2 portot !');
      Application.terminate;
      Exit;
    end;
  ...
  if not SetupComport then
    begin
      CloseComport;
      ShowMessage('Nem sikerült beállítani a COM2 portot !');
      Application.Terminate;
      Exit;
    end;
...
procedure TFnyUjsag.CsakszovegFileIras;

var _sz: string;
    _textiras: textfile;

begin
  _sz := 'KEDVEZÕ ÁRFOLYAMOK ! ÉRDEKLÖDJÖN PÉNTÁROSUNKNÁL !  ***  ';

  AssignFile(_textiras,_comPath);
  rewrite(_textiras);
  write(_textiras,_initsor+'\FS'+chr(48+_speed));
  write(_textiras,_sz+_endchar);
...
begin
 ...
 _valsor := 'CHF: '+ arfform(_chfvet)+'/'+arfform(_chfelad);
  ...
  Rewrite(_iras);
  Blockwrite(_iras,_bytetomb,_messlen);
  CloseFile(_iras);
end;
...
begin
  AssignFile(_binolvas,_comPath);
  reset(_binolvas);
  _fileMeret := fileSize(_binolvas);
  Blockread(_binolvas,_bytetomb,_filemeret);
  CloseFile(_binolvas);
  _messlen := 1;
  while _messlen<=_filemeret do
    begin
      _kod := _bytetomb[_messlen];
      if _kod=254 then break;
      inc(_messlen);
    end;
  ...
  _message := '';
  _qq := 1;
  while _qq<=_messlen do
    begin
      _message := _message + chr(_bytetomb[_qq]);
      inc(_qq);
    end;

  WriteFile(ComFile, _message[1], _messlen, _BytesWritten, nil);
end;
