...
var
  ...
  _eurvet,_plnvet,_usdvet,_czkvet: real;
  _eurelad,_plnelad,_usdelad,_czkelad: real;
  _pcs,_mondat: string;
  ...
  _sebesseg,_bytefileHossz: integer;
  _speed: integer;
...
begin
  ...
  _pcs := 'SELECT * FROM ARFOLYAM WHERE VALUTANEM='+chr(39)+'CZK'+chr(39);
  Arfnyitas(_pcs);
  _czkvet := ibQuery.fieldByName('VETELIARFOLYAM').asFloat;
  _czkelad:= ibQuery.FieldByName('ELADASIARFOLYAM').asFloat;

   _pcs := 'SELECT * FROM ARFOLYAM WHERE VALUTANEM='+chr(39)+'PLN'+chr(39);
  Arfnyitas(_pcs);
  _plnvet := ibQuery.fieldByName('VETELIARFOLYAM').asFloat;
  _plnelad:= ibQuery.FieldByName('ELADASIARFOLYAM').asFloat;
  ibQuery.close;
  ibdbase.close;
end;
...
begin
  ...
  _valsor := 'CZK: '+ arfform(_czkvet)+'/'+arfform(_czkelad);
  Valsorbeiro(_valsor);

 _valsor := 'PLN: '+ arfform(_plnvet)+'/'+arfform(_plnelad);
  Valsorbeiro(_valsor);
...
begin
  _str := floattostr(_real);
  while length(_str)<5 do _str := ' ' + _str;
  result := leftstr(_str,3)+','+midstr(_str,4,2);
