...
type
  TFnyUjsag = class(TForm)
    ...
    procedure ArfolyamFileFeliras;
    procedure TextFileFeliras;
...
var
  ...
  _csakarf,_csaktext,_vegyes: boolean;

//  _portname2: pchar = pchar('COM2:');

  _penztar,_lezartnap,_reklamtext: string;
...
begin
 ...
 if length(_parameters)<>4 then
  ...
  if (_ff=2) or (_ff=4) or (_ff=6) then _portdarab := 2
  else _portdarab := 1;
  ...
  if (_vegyes)  or (_csaktext) then
    begin
     ibDbase.connected := true;
     _pcs := 'SELECT * FROM MEDIA';
     with ibQuery do
       begin
         Close;
         Sql.Clear;
         Sql.Add(_pcs);
         Open;
         _reklamtext := trim(FieldByName('FENYTEXT').asstring);
         CLose;
       end;
     ibdbase.close;
    end;
...
begin
  ...
  ArfolyamBeolvasas;
  if _csakArf then ArfolyamFilefeliras;
  if _csaktext then TextfileFeliras;
  if (_vegyes) and (_lezartnap='') then TextfileFeliras;
  if (_vegyes) and (_lezartnap<>'') then ArfolyamFilefeliras;
...
// =============================================================================
                      procedure TFnyujsag.ArfolyamFileFelIras;
// =============================================================================
...
begin
 ...
 _valsor := 'CHF: '+ arfform(_chfvet)+'/'+arfform(_chfelad);
  ...
  if _postterm=1 then
    begin
      _valsor := 'BANKKÁRTYÁVAL IS FIZETHET';
      Valsorbeiro(_valsor);
    end;
...
// =============================================================================
                      procedure TFnyujsag.TextFileFelIras;
// =============================================================================
