-- V69: Tisza Sarok seed data - verified against production schema
-- Schema source: information_schema.columns query on production DB (2026-03-13)
--
-- branch: id=UUID(no default), NOT NULL: address,bank_code,city,code,created_at,name,
--         opening_date,zip_code,branch_status_did,branch_type_did,company_id,country_did
-- worker: id=BIGSERIAL(auto), NOT NULL: is_active,code,created_at,name,password_hash,role,branch_id,company_id
-- dictionary: id=UUID(no default), NOT NULL: category,code,created_at,name
-- company: id=UUID(no default), NOT NULL: code,created_at,name
...
-- Workers: password_hash = BCrypt('1234'), column is_active matches @Column(name="is_active") in Worker.java
