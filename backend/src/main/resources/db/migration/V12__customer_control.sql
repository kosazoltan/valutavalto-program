...
-- Ügyfél korlátozások
CREATE TABLE IF NOT EXISTS customer_restriction (
...
CREATE INDEX IF NOT EXISTS idx_customer_restriction_customer ON customer_restriction(customer_id);
CREATE INDEX IF NOT EXISTS idx_customer_restriction_type ON customer_restriction(restriction_type);
CREATE INDEX IF NOT EXISTS idx_customer_restriction_active ON customer_restriction(active);

-- Ügyfél szűrés napló (AML / szankciós lista ellenőrzés)
CREATE TABLE IF NOT EXISTS customer_screening_log (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    customer_id BIGINT NOT NULL REFERENCES customer(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_cust_screening_log_customer ON customer_screening_log(customer_id);
CREATE INDEX IF NOT EXISTS idx_cust_screening_log_type ON customer_screening_log(screening_type);
CREATE INDEX IF NOT EXISTS idx_cust_screening_log_result ON customer_screening_log(result);
