-- V74: Missing critical tables — entities that exist in Java but had no CREATE TABLE migration
-- These tables were previously created by Hibernate ddl-auto=update, now explicit.
-- Covers: CRITICAL group (referenced by ALTER/INSERT in other migrations) + BUSINESS LOGIC group
