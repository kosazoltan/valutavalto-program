...
public class BlacklistController {
    ...
    @PostMapping("/persons")
    @PreAuthorize("hasAnyRole('SUPERVISOR', 'MANAGER', 'ADMIN')")
    public ResponseEntity<ProhibitedPerson> createPerson(@Valid @RequestBody ProhibitedPerson entity) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.createPerson(entity));
    }

    @PutMapping("/persons/{id}")
    @PreAuthorize("hasAnyRole('SUPERVISOR', 'MANAGER', 'ADMIN')")
    public ResponseEntity<ProhibitedPerson> updatePerson(@PathVariable UUID id, @Valid @RequestBody ProhibitedPerson entity) {
    ...
    @PreAuthorize("hasAnyRole('SUPERVISOR', 'MANAGER', 'ADMIN')")
    public ResponseEntity<ProhibitedCompany> createCompany(@Valid @RequestBody ProhibitedCompany entity) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.createCompany(entity));
    }

    @PutMapping("/companies/{id}")
    @PreAuthorize("hasAnyRole('SUPERVISOR', 'MANAGER', 'ADMIN')")
    public ResponseEntity<ProhibitedCompany> updateCompany(@PathVariable UUID id, @Valid @RequestBody ProhibitedCompany entity) {
