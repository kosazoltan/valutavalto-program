...
public class VoiceTokenService {
     ...
     */
    public VoiceTokenResponseDto requestEphemeralToken(String mode) {
        if (!voiceEnabled) {
        ...
        if (mode == null || (!mode.equals("install") && !mode.equals("test") && !mode.equals("support"))) {
            throw new ValidationException("Érvénytelen mód: " + mode + ". Megengedett: install, test, support.");
        }
