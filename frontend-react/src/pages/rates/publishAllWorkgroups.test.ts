...
describe('publishAllWorkgroups — FK13 engedélyezett 0 árfolyam', () => {
  ...
  it.each(['0abc', '0,00oops', '0x10'])(
    'Codex fix-kör LOW: elejéről 0-nak olvasható sérült overlay („%s”) engedélyezett-nulla irányon explicit csoport-hiba, nincs publish',
    async (corrupt) => {
      localStorage.setItem(
        'arfolyamkeszito.workgroupSheet.rates.v1.g1',
        JSON.stringify({ '21.buyRate': corrupt }),
      )
      ...
      expect(result.outcomes[0]).toEqual(
        expect.objectContaining({
          status: 'failed',
          failureKind: 'business',
          message: expect.stringContaining(`érvénytelen tárolt érték: UAH buyRate („${corrupt}”)`),
        }),
      )
    },
  )
