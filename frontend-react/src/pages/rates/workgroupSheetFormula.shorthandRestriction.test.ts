...
describe('evaluateWorkgroupFormula — FR-8 shorthand L/M-megszorítás', () => {
  it('E/F/C engedélyezve (L/M oszlop) → kiértékelődik', () => {
    expect(evaluateWorkgroupFormula('F', ctx, { allowSheet0Shorthand: true })).toEqual({
      value: 390,
    })
    expect(evaluateWorkgroupFormula('F+1', ctx, { allowSheet0Shorthand: true })).toEqual({
      value: 391,
    })
    expect(evaluateWorkgroupFormula('E*0.97', ctx, { allowSheet0Shorthand: true })).toEqual({
      value: 380 * 0.97,
    })
  })
  ...
  it('E/F/C NEM engedélyezve (pl. N sáv-oszlop) → VV-VALID-004 hiba', () => {
    ...
    expect(evaluateWorkgroupFormula('F+1', ctx, { allowSheet0Shorthand: false })).toHaveProperty(
      'error',
    )
    expect(evaluateWorkgroupFormula('C', ctx, { allowSheet0Shorthand: false })).toHaveProperty(
      'error',
    )
    expect(evaluateWorkgroupFormula('E', ctx, { allowSheet0Shorthand: false })).toHaveProperty(
      'error',
    )
  })
  ...
  it('A/B/G/H/I 0-s lap self-ref NEM korlátozott (J=A funkció nem törhet)', () => {
    ...
    expect(evaluateWorkgroupFormula('A', ctx, { allowSheet0Shorthand: false })).toEqual({
      value: 100,
    })
    expect(evaluateWorkgroupFormula('G', ctx, { allowSheet0Shorthand: false })).toEqual({
      value: 7,
    })
  })

  it('!<oszlop><KÓD> (sheet0Cross) és #NN (wgCross) mindenhol érvényesek', () => {
    expect(evaluateWorkgroupFormula('!FEUR', ctx, { allowSheet0Shorthand: false })).toEqual({
      value: 400,
    })
    expect(evaluateWorkgroupFormula('#01L', ctx, { allowSheet0Shorthand: false })).toEqual({
      value: 382,
    })
  })

  it('csoport self-ref (J–S, wgSelf) mindenhol érvényes', () => {
    expect(evaluateWorkgroupFormula('L', ctx, { allowSheet0Shorthand: false })).toEqual({
      value: 381,
    })
  })
