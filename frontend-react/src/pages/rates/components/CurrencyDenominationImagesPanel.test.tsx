...
const eur: Currency = { id: 7, code: 'EUR', name: 'Euro', decimals: 2, displayOrder: 1, active: true }
...
beforeEach(() => {
  ...
  vi.mocked(currencyDenominationImageApi.getThumbnail).mockResolvedValue(new Blob(['t'], { type: 'image/jpeg' }))
})
...
describe('CurrencyDenominationImagesPanel', () => {
  ...
  it('upload_negative_faceValue_keeps_button_disabled', async () => {
    ...
    const file = new File(['bytes'], '500.jpg', { type: 'image/jpeg' })
    fireEvent.change(screen.getByTestId('denomination-image-file'), { target: { files: [file] } })
    expect((screen.getByTestId('denomination-image-upload-button') as HTMLButtonElement).disabled).toBe(true)
  })
  ...
  it('upload_success_calls_api_refreshes_list_and_clears_form', async () => {
    vi.mocked(currencyDenominationImageApi.list).mockResolvedValueOnce([]).mockResolvedValueOnce([dto])
    ...
    expect((screen.getByTestId('denomination-image-file') as HTMLInputElement).files?.length).toBe(0)
  })
  ...
  it('upload_failure_shows_error_toast_and_keeps_form_input', async () => {
    ...
    conflict.response = { status: 400, data: { code: 'VV-VALID-003', message: backendMessage } } as AxiosError['response']
    ...
    expect((screen.getByTestId('denomination-image-file') as HTMLInputElement).files?.[0]).toBe(file)
    expect((screen.getByTestId('denomination-image-upload-button') as HTMLButtonElement).disabled).toBe(false)
  })

  it('active_toggle_calls_setActive_and_refreshes', async () => {
    vi.mocked(currencyDenominationImageApi.list).mockResolvedValueOnce([dto]).mockResolvedValueOnce([{ ...dto, active: false }])
    ...
    fireEvent.click(screen.getByTestId('denomination-image-toggle-11111111-2222-3333-4444-555555555555'))
    await waitFor(() =>
  ...
  it('active_toggle_double_click_is_guarded_before_re_render', async () => {
    ...
    const toggle = screen.getByTestId('denomination-image-toggle-11111111-2222-3333-4444-555555555555')
  ...
  it('thumbnail_blob_object_url_is_created_and_revoked_on_unmount', async () => {
    ...
    await waitFor(() => expect(currencyDenominationImageApi.getThumbnail).toHaveBeenCalledWith(dto.id))
  ...
  it('currency_change_refetches_list_for_new_currencyId', async () => {
    ...
    await waitFor(() => expect(currencyDenominationImageApi.list).toHaveBeenCalledWith(7))
    const usd: Currency = { id: 8, code: 'USD', name: 'Dollar', decimals: 2, displayOrder: 2, active: true }
  ...
  it('currency switch revokes stale object URLs and re-fetches fresh thumbnails', async () => {
    ...
    const { rerender } = render(<CurrencyDenominationImagesPanel currency={eur} />)
    await waitFor(() =>
      expect(screen.getByAltText('EUR 500 Előlap')).toHaveAttribute('src', 'blob:http://test/eur-first'),
    )
    ...
    expect(screen.getByAltText('USD 100 Előlap')).not.toHaveAttribute('src', 'blob:http://test/eur-first')

    rerender(<CurrencyDenominationImagesPanel currency={eur} />)
    await waitFor(() =>
      expect(screen.getByAltText('EUR 500 Előlap')).toHaveAttribute('src', 'blob:http://test/eur-second'),
    )
    expect(screen.getByAltText('EUR 500 Előlap')).not.toHaveAttribute('src', 'blob:http://test/eur-first')
  })
})
