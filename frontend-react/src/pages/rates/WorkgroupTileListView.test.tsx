...
describe('WorkgroupTileListView (FK-02 karbantartás)', () => {
  ...
  it('írásjoggal megjeleníti az „Új munkacsoport”, Szerk. és Törlés akciókat', () => {
    ...
    expect(
      screen.getByRole('checkbox', { name: /Árfolyamvédelem a\(z\) Budapest központ/ }),
    ).toBeChecked()
  })
  ...
  it('Szerk. → átnevezés a meglévő kóddal/limitekkel hívja az update API-t', async () => {
    ...
    const dialog = screen
      .getByText('Munkacsoport átnevezése', { selector: 'h3' })
      .closest('div') as HTMLElement
    const scope = within(dialog)
    fireEvent.change(scope.getByPlaceholderText('pl. Budapest központ'), {
      target: { value: 'BP-átnevezve' },
    })
    fireEvent.click(scope.getByText('Mentés'))
    await waitFor(() =>
  ...
  it('védelem-checkbox kikapcsolása update-et hív protectionEnabled:false-szal', async () => {
    ...
    await waitFor(() =>
      expect(mocks.update).toHaveBeenCalledWith(
        'wg1',
        expect.objectContaining({ protectionEnabled: false }),
      ),
    )
  })
})
