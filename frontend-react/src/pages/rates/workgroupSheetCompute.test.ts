...
function input(
  over: Partial<WorkgroupComputeInput> & { rows: WgComputeRow[] },
): WorkgroupComputeInput {
  return {
    formulas: {},
...
describe('recomputeWorkgroupSheet (FK-04/C)', () => {
    ...
    const rows: WgComputeRow[] = [
      {
        currencyId: 1,
        currencyCode: 'EUR',
        values: { ...emptyVals(), officialRate: 400, buyRate: 395, sellRate: 405 },
      },
    ]
    ...
    const rows: WgComputeRow[] = [
      {
        currencyId: 1,
        currencyCode: 'EUR',
        values: { ...emptyVals(), officialRate: 400, sellRate: 410 },
      },
    ]
  ...
  it('0-s lap A–I self hivatkozás: J = A (elszámoló a 0-s lapról)', () => {
    ...
    const res = recomputeWorkgroupSheet(
      input({ rows, sheet0ByCurrency: sheet0, formulas: { '1.officialRate': 'A' } }),
    )
  ...
  it('FR-8: F shorthand csak L/M-ben — buyRate (L) kiértékelődik, limit1BuyRate (N) hibázik', () => {
    ...
    const rows = (): WgComputeRow[] => [
      { currencyId: 1, currencyCode: 'EUR', values: { ...emptyVals(), officialRate: 400 } },
    ]
    // F a buyRate (L) mezőben → engedélyezett shorthand → a Főlap EUR F értéke (390)
    const resL = recomputeWorkgroupSheet(
      input({ rows: rows(), sheet0ByCurrency: sheet0, formulas: { '1.buyRate': 'F' } }),
    )
    ...
    const resN = recomputeWorkgroupSheet(
      input({ rows: rows(), sheet0ByCurrency: sheet0, formulas: { '1.limit1BuyRate': 'F' } }),
    )
  ...
  it('!<oszlop><KÓD> kereszt-valuta: EUA eladása = EUR F oszlopa a 0-s lapon', () => {
    const rows: WgComputeRow[] = [
      { currencyId: 9, currencyCode: 'EUA', values: { ...emptyVals(), officialRate: 400 } },
    ]
    const sheet0 = new Map<string, Sheet0Values>([['EUR', { F: 415 }]])
    const res = recomputeWorkgroupSheet(
      input({ rows, sheet0ByCurrency: sheet0, formulas: { '9.sellRate': '!FEUR' } }),
    )
    ...
    const rows: WgComputeRow[] = [
      {
        currencyId: 1,
        currencyCode: 'EUR',
        values: { ...emptyVals(), officialRate: 400, buyRate: 100 },
      },
    ]
  ...
  it('JPY → 4 tizedes kerekítés (FK02-E FR-10 / NFR-3)', () => {
    const rows: WgComputeRow[] = [
      { currencyId: 5, currencyCode: 'JPY', values: { ...emptyVals(), officialRate: 2 } },
    ]
  ...
  it('hibás képlet (ismeretlen valuta) → errors-ban, érték marad', () => {
    const rows: WgComputeRow[] = [
      { currencyId: 1, currencyCode: 'EUR', values: { ...emptyVals(), buyRate: 395 } },
    ]
