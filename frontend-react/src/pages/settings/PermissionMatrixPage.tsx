...
export default function PermissionMatrixPage() {
    ...
    const permissionsByModule = useMemo(() => {
        ...
        for (const p of permissions) {
            if (filter && !p.name.toLowerCase().includes(filter.toLowerCase()) &&
                ...
                !p.module.toLowerCase().includes(filter.toLowerCase())) continue
    ...
    const dirtyRoles = useMemo(() => {
        ...
        for (const r of roles) {
            ...
            if (orig.size !== curr.size) { dirty.add(r.id); continue }
            for (const id of curr) if (!orig.has(id)) { dirty.add(r.id); break }
        }
        return dirty
                                    ...
                                    <tr key={r.id} className={rowIdx % 2 === 0 ? 'bg-white' : 'bg-secondary-50/50'}>
                                        ...
                                        {permissionsByModule.map(([_module, perms]) =>
                                            perms.map(p => {
                                                ...
                                                return (
                                                    <td key={p.id} className="text-center px-2 py-2 border-b border-secondary-100">
                                                        <input
                                                            type="checkbox"
                                                            checked={checked}
