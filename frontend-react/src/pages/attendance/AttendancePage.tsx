...
function MyAttendanceTab({ from, to }: { from: string; to: string }) {
    ...
    useEffect(() => {
        ...
        setLoading(true)
        setErr(null)
        workerAttendanceApi.my(from, to, 0, 100)
            .then(r => { if (!cancelled) setEntries(r.content) })
            .catch(e => {
                if (!cancelled) {
...
function TeamAttendanceTab({ from, to }: { from: string; to: string }) {
    ...
    useEffect(() => {
        ...
        setLoading(true)
        setErr(null)
        workerAttendanceApi.byWorker(selectedWorkerId, from, to, 0, 100)
            .then(r => { if (!cancelled) setEntries(r.content) })
            .catch(e => {
                if (!cancelled) {
                            ...
                            return (
                                <tr key={e.id} className={isActive ? 'bg-green-50' : ''}>
                                    ...
                                    <td className="text-sm font-mono">
                                        {e.logoutAt ? formatDateTime(e.logoutAt) : (
...
function SummaryCard({
    ...
    return (
        ...
        >
            ...
            <div className={`text-xl font-bold ${danger ? 'text-red-600' : 'text-secondary-900'}`}>{value}</div>
        </div>
    )
}
