    ...
    <div className="max-w-7xl mx-auto space-y-2">
      <div>
        ...
        <p className="text-xs text-slate-600">
          {t(
            'mnb.haviPenzvaltoForgalmiJelentesAzMnbNekGeneralasUtanEllenorizdMajdKattintsBekuldRe',
          )}
        </p>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-4">
        ...
        <div className="flex items-end gap-3">
          <div>
            <label className="block text-xs text-slate-600 mb-1" htmlFor="year">
              {t('decade.ev')}
            </label>
            <select
              id="year"
          ...
          <div>
            <label className="block text-xs text-slate-600 mb-1" htmlFor="month">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {reports.map((r) => (
                <tr
                  key={r.id}
                  className={`border-t border-slate-100 hover:bg-slate-50 ${r.status === 'REJECTED' ? 'bg-red-50' : ''}`}
                >
                  <td className="px-4 py-3 font-mono">{getReportPeriodLabel(r)}</td>
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded border ${STATUS_COLORS[r.status]}`}
                    >
                      {statusIcon(r.status)} {STATUS_LABELS[r.status]}
                    </span>
                    {r.retryCount && r.retryCount > 0 && (
                      <span className="ml-2 text-xs text-slate-500">
                        ×{r.retryCount} {t('mnb.retry')}
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3 font-mono text-xs text-slate-600">
                    {r.mnbReferenceNumber || '—'}
                  </td>
                  <td className="px-4 py-3 text-xs text-slate-500">
                    {r.createdAt ? new Date(r.createdAt).toLocaleDateString('hu-HU') : '—'}
                  </td>
                  <td className="px-4 py-3 text-xs text-slate-500">
                    {r.submittedAt ? new Date(r.submittedAt).toLocaleString('hu-HU') : '—'}
                  </td>
