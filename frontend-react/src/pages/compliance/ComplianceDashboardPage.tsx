...
import { amlApi, RollingWindowAuditDto, AmlDailySummary, AmlReportDto, AmlCheckResult } from '../../services/api/aml'
            ...
            {loading ? (
                ...
                <>
                    ...
                    <div className="bg-white shadow rounded overflow-hidden mb-6">
                        ...
                        <form onSubmit={(event) => void handleManualCheck(event)} className="grid grid-cols-1 gap-3 p-4 lg:grid-cols-[1.2fr_1fr_0.7fr_auto] lg:items-end">
                            <div>
                                <label className="form-label">Ügyfél azonosító</label>
                                <input
                                    type="text"
                                    value={manualCheckForm.customerId}
                                    onChange={(event) => setManualCheckForm(prev => ({ ...prev, customerId: event.target.value }))}
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(event) => setManualCheckForm(prev => ({ ...prev, amountHuf: event.target.value }))}
                                    className="form-input font-mono"
                                    required
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(event) => setManualCheckForm(prev => ({ ...prev, currencyCode: event.target.value.toUpperCase() }))}
                                    ...
                                    <div
                                        className={`rounded border p-3 text-sm ${
                                            manualCheckResult.blocked
                                                ...
                                                : manualCheckResult.requiresManagerApproval || manualCheckResult.requiresEnhanced
                                    ...
                                    >
                                        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                                            ...
                                            <div className="font-mono">
                                                8 nap: {manualCheckResult.rollingWindowTotal?.toLocaleString('hu-HU') ?? 0} / {manualCheckResult.rollingWindowLimit?.toLocaleString('hu-HU') ?? 0} HUF
                                            </div>
                                        </div>
                                        <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-3">
                                            <span>{manualCheckResult.blocked ? 'Blokkolt' : 'Nem blokkolt'}</span>
                                            <span>{manualCheckResult.requiresId ? 'Azonosítás kell' : 'Azonosítás nem jelzett'}</span>
                                            <span>{manualCheckResult.requiresManagerApproval ? 'Vezetői jóváhagyás kell' : 'Vezetői jóváhagyás nem jelzett'}</span>
                                        </div>
                                        {manualCheckResult.managerApprovalReason && (
                                            <div className="mt-2 font-medium">{manualCheckResult.managerApprovalReason}</div>
                                        )}
                                    ...
                                    <tbody>
                                        {overdue.slice(0, 10).map(r => (
                                            <tr key={r.id} className="border-t">
                                                ...
                                                <td className="px-4 py-2 text-right font-mono">{formatNumber(r.amountHuf)}</td>
