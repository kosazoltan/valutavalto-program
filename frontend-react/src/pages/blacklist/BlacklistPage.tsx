...
const emptyPersonForm: PersonForm = { fullName: '', documentNumber: '', identityNumber: '', passportNumber: '', dateOfBirth: '', nationality: '', listType: 'INTERNAL', listSource: 'MANUAL', reason: '' }
const emptyCompanyForm: CompanyForm = { companyName: '', taxNumber: '', registrationNumber: '', listType: 'INTERNAL', listSource: 'MANUAL', reason: '' }
...
export default function BlacklistPage() {
  ...
  const [stats, setStats] = useState({ totalPersons: 0, totalCompanies: 0, activePersons: 0, activeCompanies: 0 })
  ...
  const handleSavePerson = async () => {
    if (!personForm.fullName) { toast.warning('Hiányzó adat', 'Név megadása kötelező'); return }
    try {
      setSaving(true)
      setError(null)
      if (editingPersonId) {
  ...
  const handleSaveCompany = async () => {
    if (!companyForm.companyName) { toast.warning('Hiányzó adat', 'Cégnév megadása kötelező'); return }
    try {
      setSaving(true)
      setError(null)
      if (editingCompanyId) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 items-center">
        ...
        <input className="form-input flex-1" placeholder="Keresés név, okmányszám, adószám alapján..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      </div>
                ...
                <tbody>
                  {filteredPersons.map(p => (
                    <tr key={p.id}>
                      ...
                      <td>{p.dateOfBirth || '-'}</td>
                      <td>{p.nationality || '-'}</td>
                      <td><span className="badge badge-red">{LIST_TYPES.find(t => t.value === p.listType)?.label || p.listType}</span></td>
                      <td className="text-sm">{p.listSource}</td>
                      <td><span className={`badge ${p.isActive ? 'badge-red' : 'badge-gray'}`}>{p.isActive ? 'Aktív' : 'Inaktív'}</span></td>
                      <td>
                        <div className="flex gap-1">
                          <button onClick={() => handleEditPerson(p)} className="form-button text-xs" title="Szerkesztés"><Edit2 size={12} /></button>
                          <button onClick={() => handleDeletePerson(p.id)} className="form-button text-xs text-red-600" title="Törlés"><Trash2 size={12} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
                ...
                <tbody>
                  {filteredCompanies.map(c => (
                    <tr key={c.id}>
                      ...
                      <td><span className="badge badge-red">{LIST_TYPES.find(t => t.value === c.listType)?.label || c.listType}</span></td>
                      <td className="text-sm">{c.listSource}</td>
                      <td><span className={`badge ${c.isActive ? 'badge-red' : 'badge-gray'}`}>{c.isActive ? 'Aktív' : 'Inaktív'}</span></td>
                      <td>
                        <div className="flex gap-1">
                          <button onClick={() => handleEditCompany(c)} className="form-button text-xs" title="Szerkesztés"><Edit2 size={12} /></button>
                          <button onClick={() => handleDeleteCompany(c.id)} className="form-button text-xs text-red-600" title="Törlés"><Trash2 size={12} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
