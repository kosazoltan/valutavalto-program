...
describe('Production API — Worker first-time setup validation', () => {
  ...
  it('Ismeretlen workerCode → 400 hibaválasz', async () => {
  ...
  it('Ismeretlen companyCode → 400 + hibaüzenet', async () => {
    const res = await fetch(`${API_BASE}/auth/first-time-worker-setup`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  it('Hiányzó newPassword → 400 (validációs hiba)', async () => {
    const res = await fetch(`${API_BASE}/auth/first-time-worker-setup`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    // A backend validálja a DTO-t — hiányzó mező → 400
    expect(res.status).toBe(400)
  })
})
...
describe('Production API — SetupWizard teljes flow szimuláció', () => {
  it('A teljes flow lépései mind sikeresen hívhatók', async () => {
    ...
    // A válasz 200 (success) vagy 400 (validation) — de NEM 404, 500, 502, stb.
