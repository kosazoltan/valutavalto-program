...
export default function TransitPage() {
  ...
  const branchCode = useMemo(() => {
    ...
    const fromEnv = String(import.meta.env.VITE_BRANCH_CODE ?? '')
      .trim()
      .toUpperCase()
  ...
  const handleAcknowledge = async (item: TransitItem) => {
    ...
    try {
      ...
      toast.success(
        'Atvetel validalva',
        item.documentNumber + ' - ' + item.amount.toLocaleString('hu-HU') + ' ' + item.currencyCode,
      )
      await load()
    } catch (err) {
    ...
    <div>
      ...
      {items.length === 0 && !loading ? (
        <div className="text-center py-12 text-secondary-500">
          ...
          <p>
            {t('rate-management.nincs')} {tab === 'incoming' ? 'érkező' : 'kimenő'}{' '}
            {t('transit.utonLevoCsomag')}
          </p>
        </div>
      ) : (
                      ...
                      <span
                        className={
                          item.status === 'IN_PROGRESS'
                            ? 'px-2 py-0.5 bg-amber-100 text-amber-800 rounded-full'
                            : 'px-2 py-0.5 bg-secondary-100 text-secondary-700 rounded-full'
                        }
                      >
      ...
      <p className="mt-4 text-xs text-secondary-500">
        {t(
          'transit.automatikusFrissites30MasodpercenkentAtvetelKattintassalValidalhatoEzzelCsokkentjukACsomagveszitesKockazatat',
        )}
      </p>
    </div>
  )
