...
describe('transferRules — átadás-átvétel üzleti szabályok', () => {
  ...
  describe('getAllowedTransferTypeValues (DRY — single source of truth)', () => {
    ...
    it('konzisztens a getAvailableTransferTypes value-listájával (nincs drift)', () => {
      for (const vault of [true, false]) {
        expect(getAllowedTransferTypeValues(vault)).toEqual(
          getAvailableTransferTypes(vault, 'in').map((o) => o.value),
        )
      }
    })
  })
  ...
  describe('filterTransferTargetBranches (#1 cél-fiók szűrés)', () => {
    ...
    it('isMainCashierBranch SZŰK: nem match-el general nevet', () => {
      expect(isMainCashierBranch({ id: 'x', code: 'BR500', name: 'Pécs Plaza pénztár' })).toBe(
        false,
      )
      expect(isMainCashierBranch({ id: 'f', code: 'BR001', name: 'Egyes számú pénztár' })).toBe(
        true,
      )
    ...
    it('üres-fallback: ha a szűrő 0 fiókot adna, MINDET visszaadja', () => {
      const onlyPlain = [
        { id: 'x', code: 'BR999', name: 'Sima Pénztár', region: 'PECS', isVault: false },
      ]
...
describe('validateCarrierSeal — átadás-átvétel szállító + plombaszám (FK02)', () => {
  ...
  it('128 karakternél hosszabb szállító név esetén hibát ad', () => {
    expect(validateCarrierSeal('a'.repeat(129), 'ABC-1')).toBe(
      'A szállító neve legfeljebb 128 karakter lehet!',
    )
  })

  it('64 karakternél hosszabb plombaszám esetén hibát ad', () => {
    expect(validateCarrierSeal('Brink', 'A'.repeat(65))).toBe(
      'A plombaszám legfeljebb 64 karakter lehet!',
    )
  })

  it('érvénytelen karaktert tartalmazó plombaszám esetén hibát ad', () => {
    expect(validateCarrierSeal('Brink', 'ABC 123')).toBe(
      'A plombaszám csak betűt, számot, kötőjelet és perjelet tartalmazhat!',
    )
    expect(validateCarrierSeal('Brink', 'ABC#1')).toBe(
      'A plombaszám csak betűt, számot, kötőjelet és perjelet tartalmazhat!',
    )
  })
  ...
  describe('buildDenominationPayload (FR-17..20b — címletezés)', () => {
    ...
    it('FR-19: bekapcsolt, de üres/érvénytelen sorok → nincs címletezés', () => {
      const r = buildDenominationPayload(
        true,
        [
          { quantity: '', faceValue: '' },
          { quantity: '0', faceValue: '100' },
        ],
        1000,
      )
      expect(r.denominations).toBeUndefined()
