      ...
      {loading ? (
        ...
        <div className="space-y-4">
          <div className="text-sm text-gray-600">
            {data.branchCode} – {data.branchName} | {data.yearMonth} | Munkanapok:{' '}
            {data.workingDays} (lezárt: {data.closedDays})
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="p-3 bg-white border rounded">
              <div className="text-xs text-gray-500">Összes vétel (Ft)</div>
              <div className="font-bold">{fmt(data.totalBuyHuf)}</div>
            </div>
            <div className="p-3 bg-white border rounded">
              <div className="text-xs text-gray-500">Összes eladás (Ft)</div>
              <div className="font-bold">{fmt(data.totalSellHuf)}</div>
            </div>
            <div className="p-3 bg-white border rounded">
              <div className="text-xs text-gray-500">Készpénzes forgalom</div>
              <div className="font-bold">{fmt(data.cashTurnoverHuf)}</div>
            </div>
            <div className="p-3 bg-white border rounded">
              <div className="text-xs text-gray-500">Bankkártyás forgalom</div>
              <div className="font-bold">{fmt(data.cardTurnoverHuf)}</div>
            </div>
            <div className="p-3 bg-white border rounded">
              <div className="text-xs text-gray-500">Tranzakciók</div>
              <div className="font-bold">
                {data.transactionCount} (V:{data.buyCount} E:{data.sellCount} S:{data.reversalCount}
                )
              </div>
            </div>
            <div className="p-3 bg-white border rounded">
              <div className="text-xs text-gray-500">Kezelési díj</div>
              <div className="font-bold">{fmt(data.handlingFeeBalance)} Ft</div>
            </div>
            <div className="p-3 bg-white border rounded">
              <div className="text-xs text-gray-500">WU egyenleg</div>
              <div className="font-bold">{fmt(data.wuHufBalance)} Ft</div>
            </div>
            <div className="p-3 bg-white border rounded">
              <div className="text-xs text-gray-500">ÁFA / E-ker</div>
              <div className="font-bold">
                {fmt(data.afaBalance)} / {fmt(data.ecommerceBalance)} Ft
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded border">
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="p-2 text-left">VNEM</th>
                    <th className="p-2 text-right">Átvett</th>
                    <th className="p-2 text-right">Átadott</th>
                    <th className="p-2 text-right">Nettó</th>
                  </tr>
                </thead>
                <tbody>
                  {transfers.map((tr) => (
          ...
          <div className="p-3 bg-gray-50 border rounded text-right text-sm">
            Záró készlet: HUF <strong>{fmt(data.closingBalanceHuf)}</strong> + deviza(HUF-ban){' '}
            {fmt(data.closingBalanceForeign)} = <strong>{fmt(data.closingBalanceTotal)} Ft</strong>
          </div>
        </div>
      )}
    </div>
