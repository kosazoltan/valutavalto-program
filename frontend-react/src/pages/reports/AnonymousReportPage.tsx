...
export default function AnonymousReportPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'RESOLVED': return <span className="badge badge-green"><CheckCircle size={10} className="inline" />{t('reports.lezart')}</span>
      case 'IN_PROGRESS': return <span className="badge badge-blue"><Clock size={10} className="inline" />{t('common.inProgress')}</span>
      case 'ASSIGNED': return <span className="badge badge-yellow"><UserCheck size={10} className="inline" />{t('reports.kiosztva')}</span>
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h1 className="text-xl font-bold flex items-center gap-2"><AlertCircle />{t('reports.nevtelenBejelentesek')}</h1>
        <button onClick={() => setShowNew(true)} className="form-button-primary"><Plus size={16} />{t('reports.ujBejelentes')}</button>
      </div>
      ...
      {showNew && (
        <div className="form-panel space-y-3 border-2 border-blue-200">
          ...
          <div className="flex gap-2">
            <button onClick={() => void handleCreate()} className="form-button-primary">{t('reports.rogzites')}</button>
            <button onClick={() => setShowNew(false)} className="form-button">{t('common.cancel')}</button>
          </div>
        </div>
      )}
      ...
      {selectedReport && (
        <div className="form-panel space-y-3 border-2 border-yellow-200">
          <h2 className="font-semibold flex items-center gap-2"><Eye size={18} />{t('reports.bejelentesReszletei')}</h2>
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div><span className="text-gray-500">{t('cashdesk.tipus')}</span> {selectedReport.reportType}</div>
            <div><span className="text-gray-500">{t('darius.statusz')}</span> {getStatusBadge(selectedReport.status)}</div>
            <div><span className="text-gray-500">{t('reports.targy2')}</span> {selectedReport.subject}</div>
            <div><span className="text-gray-500">{t('components.datum')}</span> {new Date(selectedReport.reportedAt).toLocaleString('hu-HU')}</div>
            <div className="col-span-2"><span className="text-gray-500">{t('reports.leiras2')}</span> {selectedReport.description || '-'}</div>
            <div><span className="text-gray-500">{t('reports.felelos')}</span> {selectedReport.assignedToName || 'Nincs'}</div>
          </div>
          {selectedReport.status !== 'RESOLVED' && (
            <div className="flex gap-2 items-end">
              ...
              <button onClick={() => void handleAssign(selectedReport.id)} className="form-button"><UserCheck size={16} />{t('reports.kiosztas')}</button>
              <button onClick={() => void handleResolve(selectedReport.id)} className="form-button-primary"><CheckCircle size={16} />{t('decade.lezaras')}</button>
            </div>
          )}
          <button onClick={() => setSelectedReport(null)} className="form-button text-sm">{t('common.close')}</button>
        </div>
      )}
      ...
      <div className="form-panel flex gap-3 items-end">
        ...
        <span className="text-sm text-gray-500">{filtered.length} {t('reports.bejelentes')}</span>
      </div>
      ...
      <div className="form-panel">
        {loading ? <div>Betöltés...</div> : filtered.length === 0 ? (
          ...
          <table className="data-grid w-full">
            <thead><tr><th>{t('common.type')}</th><th>{t('reports.targy3')}</th><th>{t('common.date')}</th><th>{t('common.status')}</th><th>{t('reports.felelos2')}</th><th>{t('common.actions')}</th></tr></thead>
            <tbody>
              {filtered.map(r => (
