...
import {
  useCurrencyCatalog,
  selectCatalogCurrencies,
  CROSS_BASE_MAP,
  getCrossBase,
} from './useCurrencyCatalog'
...
describe('FK04 — useCurrencyCatalog', () => {
  ...
  it('out_of_order_reload: a régebbi (lassabb) válasz nem írja felül az újabbat (last-requested-wins)', async () => {
    ...
    const first = new Promise<Currency[]>((r) => {
      resolveFirst = r
    })
    const second = new Promise<Currency[]>((r) => {
      resolveSecond = r
    })
