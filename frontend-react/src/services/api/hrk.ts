...
export const hrkMonthlyApi = {
  getSummary: async (yearMonth: string): Promise<HrkMonthlySummary> => {
    const response = await api.get<HrkMonthlySummary>('/hrk/monthly/summary', { params: { yearMonth } })
    return response.data
  },
  close: async (yearMonth: string): Promise<HrkMonthlySummary> => {
    const response = await api.post<HrkMonthlySummary>('/hrk/monthly/close', undefined, { params: { yearMonth } })
    return response.data
  },
}

export const hrkDailyApi = {
  handover: async (branchId: string, data: CreateHrkTransactionRequest): Promise<HrkTransaction> => {
    const response = await api.post<HrkTransaction>('/hrk/handover', data, { headers: { 'X-Branch-Id': branchId } })
    return response.data
  },
  receive: async (branchId: string, data: CreateHrkTransactionRequest): Promise<HrkTransaction> => {
    const response = await api.post<HrkTransaction>('/hrk/receive', data, { headers: { 'X-Branch-Id': branchId } })
    return response.data
  },
  getJournal: async (branchId: string): Promise<HrkTransaction[]> => {
    const response = await api.get<HrkTransaction[]>('/hrk/journal', { headers: { 'X-Branch-Id': branchId } })
    return response.data
  },
