...
export interface IncomeProofRequiredDto {
  required: boolean
  thresholdHuf: number
}
...
export const incomeSourceDocApi = {
  checkRequired: async (
    ...
    (
      await api.get<IncomeProofRequiredDto>('/income-source-docs/required', {
        params: { hufAmount, customerId, currencyCode },
      })
    ).data,
  ...
  putRecipients: async (recipients: string[]): Promise<IncomeProofRecipientsUpdateResponse> =>
    (
      await api.put<IncomeProofRecipientsUpdateResponse>('/income-source-docs/recipients', {
        recipients,
      })
    ).data,
}
