...
test('területi elszámolás mobil viewporton kezeli a territories backend szerződést', async ({
  page,
}) => {
  ...
  const profitRequest = page.waitForRequest(
    (request) => request.method() === 'GET' && request.url().includes('/territories/20/profit'),
  )
  await page.getByRole('button', { name: 'Lekérdez' }).click()
  await reportRequest
  await profitRequest
  await expect(
    page.getByText('Reconciliation OK: Σ pénztár összhaszon = terület összhaszon'),
  ).toBeVisible()
  ...
  const createRequest = page.waitForRequest(
    (request) => request.method() === 'POST' && request.url().endsWith('/api/v1/territories'),
  )
