...
const TOKEN = makeJwt({
  ...
  activeRole: 'ADMIN',
  permissions: [
    'TRADE_EXECUTE','TRADE_STORNO','CUSTOMER_READ','CUSTOMER_WRITE',
    'RATE_READ','RATE_WRITE','CASHDESK_READ','CASHDESK_WRITE',
    'REPORT_READ','CLOSING_WRITE','ADMIN','TRANSFER_WRITE',
    'SHIPMENT_READ','COMMISSION_READ','AUDIT_READ',
  ],
})

const WORKER = {
  id: 1, workerCode: 'ADMIN', firstName: 'Admin', lastName: 'Teszt',
  fullName: 'Admin Teszt', role: 'ADMIN',
  branchId: 'branch-1', branchCode: 'BUD01', branchName: 'Budapest 01',
  companyId: 'company-1', companyCode: 'EBC', companyName: 'Exclusive Best Change',
}

const EMPTY = JSON.stringify({
  content: [], data: [], items: [], total: 0,
  totalElements: 0, totalPages: 1, number: 0, size: 20,
  rates: [], transactions: [], customers: [], workers: [],
  branches: [], currencies: [],
})
...
type RouteResult = {
  route: string; loaded: boolean; error?: string; title?: string
  clicked: number; disabled: number; errors: number; skipped: number
  buttons: BtnResult[]
}
...
async function probeRoute(page: Page, route: string): Promise<RouteResult> {
  ...
  try {
    ...
    return { route, loaded: false, error: `goto timeout: ${String(e).slice(0,120)}`,
      clicked:0, disabled:0, errors:0, skipped:0, buttons:[] }
  }
  ...
  for (const b of await page.locator('button:not([disabled]):not([aria-disabled="true"])').all()) {
    ...
    try {
      ...
      if (SKIP_LABELS.some(s => key.includes(s))) {
        btns.push({ label: lbl||aria||'[no-label]', state: 'skip' }); continue
      }
      const visible = await b.isVisible().catch(() => false)
      if (!visible) { btns.push({ label: lbl||'[hidden]', state: 'skip' }); continue }
...
for (const route of ROUTES) {
  test(`route: ${route}`, async ({ page }) => {
    ...
    if (errBtns.length) {
      console.warn(`[BTN_ERR] ${route}: ${errBtns.map(b=>`"${b.label}": ${b.err}`).join(' | ')}`)
    }

    console.log(`[OK] ${route} | clicked=${result.clicked} disabled=${result.disabled} err=${result.errors} skip=${result.skipped}`)
  })
}
...
test('RELAY REPORT: aggregate all results', async () => {
  const dir = RESULTS_FILE
  if (!fs.existsSync(dir)) { console.log('No results yet'); return }

  const files = fs.readdirSync(dir).filter(f => f.endsWith('.json'))
  const all: RouteResult[] = files.map(f => JSON.parse(fs.readFileSync(path.join(dir, f), 'utf-8')))
  ...
  const bugs = [
    ...
    ...loaded.flatMap(r => r.buttons.filter(b=>b.state==='error').map(b=>`[BTN_ERR] ${r.route} → "${b.label}": ${b.err}`)),
  ]
