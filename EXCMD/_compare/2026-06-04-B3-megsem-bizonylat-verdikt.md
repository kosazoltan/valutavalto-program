...
2. **A NYOMTATÁS meghiúsul/megszakad, de a tranzakció MÁR rögzített és érvényes** → **FIZIKAI ÚJRANYOMTATÁS a függő bizonylat-vázlatból**. Codex P2-pontosítás (helyes): a backend `receiptApi.print` (`POST /api/v1/receipts/{id}/print` → `ReceiptService.print`) CSAK az `isPrinted` flaget állítja + materializál — NEM vált ki fizikai ESC-POS nyomtatást, ezért az ÖNMAGÁBAN nem a recovery. A tényleges fizikai újranyomtatás a penztar-client Electron-útvonalán történik a függő vázlatból: `ReceiptPage.tsx:228` „Vázlat nyomtatás" gomb → `ReceiptPreviewModal` (`variant="draft"`) `onPrint` (`ReceiptPage.tsx:315`/`:320`) → `localQueue.ts:195` `printPendingReceiptDraft` → `:202` `window.electronAPI.printReceipt` → IPC `print-receipt` (`penztar-client/electron/main.ts:337`) → `printReceipt()` ESC-POS/serial (`printer.ts`). A `getPendingReceiptDrafts` (Electron-only) addig listázza a vázlatot, amíg az ki nem nyomtatódott, így egy meghiúsult nyomtatás után a pénztáros a bizonylat-böngészőben újra fizikailag kinyomtathatja. NEM storno, NEM MEGSEM: a tranzakció érvényes marad, csak a fizikai nyomtatás ismétlődik (a bizonylatszám a rekordhoz tartozik, nem vész el).
