...
if (!ghStatus.ok) {
  ...
  section('GitHub CLI', [
    ...
    fence(
      redact(ghStatus.stderr || ghStatus.stdout || ghStatus.error || 'unknown gh auth failure'),
    ),
  ])
} else {
...
function collectPrChecks(prInfo) {
  ...
  if (!checks.ok) {
    section('PR Checks', [
      'Could not read `gh pr checks`.',
      fence(redact(checks.stderr || checks.error || '')),
    ])
    return
  }
...
function collectReviewSignals(repoSlug, prNumber) {
  ...
  if (comments.ok && Array.isArray(comments.data)) {
    for (const comment of comments.data.filter((item) => botPattern.test(item.user?.login ?? ''))) {
      ...
      lines.push(
        `## ${comment.user.login} inline: ${comment.path ?? '(unknown file)'}:${comment.line ?? comment.original_line ?? 0}`,
      )
      lines.push(fence(trimBlock(body, 7000)))
  ...
  section(
    'Sourcery / Codex / Copilot Reviews',
    lines.length > 0 ? lines : ['No Sourcery, Codex, or Copilot review signal was returned.'],
  )
}
...
function collectCheckRuns(repoSlug, headSha) {
  ...
  for (const runItem of runs.data.check_runs) {
    ...
    if (annotationCount > 0) {
      const annotations = runJson('gh', [
        'api',
        `/repos/${repoSlug}/check-runs/${runItem.id}/annotations?per_page=100`,
      ])
      if (annotations.ok && Array.isArray(annotations.data)) {
        ...
        for (const annotation of annotations.data.slice(0, 30)) {
          ...
          if (/failure|error/i.test(annotation.annotation_level ?? '')) {
            findings.push(
              `Check annotation: ${runItem.name} ${annotation.path ?? ''}:${annotation.start_line ?? 0}`,
            )
          }
        }
      }
  ...
  section(
    ...
    lines.length > 0 ? lines : ['No failed check-runs or annotations were returned.'],
  )
}
...
function collectWorkflowRunLogs(repoSlug, headSha) {
  const runs = runJson('gh', [
    'api',
    `/repos/${repoSlug}/actions/runs?head_sha=${headSha}&per_page=20`,
  ])
  if (!runs.ok || !Array.isArray(runs.data?.workflow_runs)) {
    section('GitHub Actions Failed Logs', [
      'Could not read workflow runs for the current head SHA.',
    ])
    return
  }
  ...
  if (failedRuns.length === 0) {
    section('GitHub Actions Failed Logs', [
      'No failed GitHub Actions workflow run was found for the current head SHA.',
    ])
    return
  }
  ...
  for (const workflowRun of failedRuns.slice(0, 8)) {
    ...
    const log = run('gh', ['run', 'view', String(workflowRun.id), '--log-failed'], {
      maxBuffer: maxLogChars + 200000,
    })
    const fallbackLog =
      log.ok && log.stdout.trim()
        ? log
        : run('gh', ['run', 'view', String(workflowRun.id), '--log'], {
            maxBuffer: maxLogChars + 200000,
          })
...
function collectLocalLogs() {
  ...
  section(
    'Codex / Local Logs',
    lines.length > 0 ? lines : ['No local error blocks were found in /tmp/logs or /tmp.'],
  )
}
...
function renderBlocks(text, label) {
  ...
  const hits = []
  const pattern =
    /(::error|error|failed|failure|exception|caused by|npm err|build failure|compilation failure|assertionerror|typeerror|referenceerror|process completed with exit code|tests run:.*failures|failures?: [1-9]|fatal)/i
...
function getPrInfo() {
  ...
  const command = prArg
    ? [
        ...
        String(prArg),
        '--json',
        'number,title,headRefOid,headRefName,baseRefName,url,state,reviewDecision,mergeStateStatus',
      ]
    : [
        'pr',
        'view',
...
function hasBlockingText(text) {
  return /(changes requested|p0|p1|bug[_ -]?risk|security|must fix|blocking|error|failure|failed)/i.test(
    text ?? '',
  )
}
...
function redact(text) {
  return String(text ?? '')
    ...
    .replace(
      /(token|secret|password|passwd|jwt|api[_-]?key|client[_-]?secret|private[_-]?key)(\s*[:=]\s*)[^\s"']+/gi,
      '$1$2[REDACTED]',
    )
