...
const PLACEHOLDER_PATTERN =
  /TODO|TBD|FIXME|KIT[ÖO]LTEND|P[ÉE]LDA|EXAMPLE|SAMPLE|CHANGE_?ME|^\s*$|^X{2,}$|^\.{3}$|<[^>]*>/i
...
function requireDate(obj, field, label, reasons, { maxAgeDays = null } = {}) {
  ...
  if (maxAgeDays != null && now - parsed.getTime() > maxAgeDays * 24 * 3600 * 1000) {
    reasons.push(
      `${label}.${field}: ${value} régebbi, mint ${maxAgeDays} nap — friss evidence kell a release-hez`,
    )
  }
  return parsed
}
...
function requireVersionMatch(obj, field, label, reasons) {
  ...
  if (value != null && value !== expected) {
    reasons.push(
      `${label}.${field} = ${value}, de a repo verziója ${expected} — az evidence nem erre a release-re vonatkozik`,
    )
  }
}
...
const AGGREGATE_VALIDATORS = {
  acceptance(aggregate) {
    ...
    if (section.status !== 'PASS')
      reasons.push(
        `${aggregate.fileName}: acceptance.status = ${JSON.stringify(section.status)} — PASS kell`,
      )
    ...
    if (section.failed !== 0)
      reasons.push(
        `${aggregate.fileName}: acceptance.failed = ${JSON.stringify(section.failed)} — 0 kell`,
      )
    if (section.criticalFlowsPassed !== true)
      reasons.push(
        `${aggregate.fileName}: acceptance.criticalFlowsPassed = ${JSON.stringify(section.criticalFlowsPassed)} — true kell`,
      )
    const env = aggregate.data.environment
    if (!['staging', 'production'].includes(env))
      reasons.push(
        `${aggregate.fileName}: environment = ${JSON.stringify(env)} — staging vagy production kell`,
      )
    return reasons
  },
  approvals(aggregate) {
  ...
  compliance(aggregate) {
    ...
    if (section.status !== 'PASS')
      reasons.push(
        `${aggregate.fileName}: compliance.status = ${JSON.stringify(section.status)} — PASS kell`,
      )
    if (section.approvedDecisionStatus !== 'APPROVED')
      reasons.push(
        `${aggregate.fileName}: compliance.approvedDecisionStatus = ${JSON.stringify(section.approvedDecisionStatus)} — APPROVED kell`,
      )
    ...
    if (!['staging', 'production'].includes(section.exportedEnvironment)) {
      reasons.push(
        `${aggregate.fileName}: compliance.exportedEnvironment = ${JSON.stringify(section.exportedEnvironment)} — staging vagy production kell`,
      )
    }
  ...
  'dr-drill'(aggregate) {
    ...
    if (section.status !== 'PASS')
      reasons.push(
        `${aggregate.fileName}: drRestore.status = ${JSON.stringify(section.status)} — PASS kell`,
      )
    ...
    if (typeof section.rtoMinutes !== 'number' || section.rtoMinutes > MAX_DR_RESTORE_MINUTES) {
      reasons.push(
        `${aggregate.fileName}: drRestore.rtoMinutes = ${JSON.stringify(section.rtoMinutes)} — szám kell, ≤ ${MAX_DR_RESTORE_MINUTES}`,
      )
    }
    if (section.flywayVerified !== true)
      reasons.push(`${aggregate.fileName}: drRestore.flywayVerified — true kell`)
    if (section.auditHashChainVerified !== true)
      reasons.push(`${aggregate.fileName}: drRestore.auditHashChainVerified — true kell`)
    return reasons
  },
  monitoring(aggregate) {
    ...
    if (section.status !== 'PASS')
      reasons.push(
        `${aggregate.fileName}: monitoring.status = ${JSON.stringify(section.status)} — PASS kell`,
      )
    requireString(section, 'reportRef', 'monitoring', reasons)
    if (
      typeof section.observationHours !== 'number' ||
      section.observationHours < MIN_MONITORING_HOURS
    ) {
      reasons.push(
        `${aggregate.fileName}: monitoring.observationHours = ${JSON.stringify(section.observationHours)} — szám kell, ≥ ${MIN_MONITORING_HOURS}`,
      )
    }
    for (const flag of ['scrapeVerified', 'dashboardLoaded', 'alertDelivered']) {
      if (section[flag] !== true)
        reasons.push(`${aggregate.fileName}: monitoring.${flag} — true kell`)
    }
    return reasons
  },
  installer(aggregate) {
    ...
    if (section.status !== 'PASS')
      reasons.push(
        `${aggregate.fileName}: installer.status = ${JSON.stringify(section.status)} — PASS kell`,
      )
    requireString(section, 'signedArtifactReportRef', 'installer', reasons)
    requireString(section, 'cleanVmReportRef', 'installer', reasons)
    if (section.signedArtifactsVerified !== true)
      reasons.push(`${aggregate.fileName}: installer.signedArtifactsVerified — true kell`)
    ...
    clients.forEach((client) => {
      for (const step of ['installed', 'launched', 'uninstalled']) {
        if (client[step] !== true) {
          reasons.push(
            `${aggregate.fileName}: installer.clients[${client.name ?? '?'}].${step} — true kell`,
          )
        }
      }
    })
  ...
  evidence(aggregate) {
    ...
    if (aggregate.data.evidenceStatus !== 'COMPLETE') {
      reasons.push(
        `${aggregate.fileName}: evidenceStatus = ${JSON.stringify(aggregate.data.evidenceStatus)} — COMPLETE kell`,
      )
    }
  ...
  if (!scenarios) {
    ...
    scenarios.forEach((scenario, i) => {
      ...
      if (scenario.result !== 'PASS') {
        reasons.push(
          `acceptance.scenarios[${i}] (${scenario.id ?? '?'}): result = ${JSON.stringify(scenario.result)} — minden forgatókönyvnek PASS kell`,
        )
      }
    })
    for (const required of REQUIRED_ACCEPTANCE_SCENARIOS) {
      if (!seen.has(required)) {
        reasons.push(
          `acceptance.scenarios: hiányzó kötelező forgatókönyv: ${required} (roadmap P0-4: vétel/eladás/sztornó/napzárás/havizárás)`,
        )
      }
    }
  }
...
function checkApprovals() {
  ...
  approvals.forEach((approval, i) => {
    ...
    if (approval.decision !== 'APPROVED') {
      reasons.push(
        `approvals[${i}] (${approval.role ?? '?'}): decision = ${JSON.stringify(approval.decision)} — APPROVED kell`,
      )
    }
...
function checkCompliance() {
  ...
  if (!dbExport || typeof dbExport !== 'object') {
    ...
    requireDate(dbExport, 'exportedAt', 'compliance.dbExport', reasons, {
      ...
      if (inRepo.startsWith(repoRoot + path.sep)) {
        ...
        if (fileBytes != null) {
          ...
          if (actual.toLowerCase() !== sha.toLowerCase()) {
            reasons.push(
              `compliance.dbExport: a(z) ${location} tényleges SHA-256-ja (${actual}) NEM egyezik a megadottal`,
            )
          }
        }
      }
  ...
  const decisions = Array.isArray(data.enforcementFlagDecisions)
    ? data.enforcementFlagDecisions
    : []
  const decidedFlags = new Map()
  decisions.forEach((decision, i) => {
    const flag = requireString(
      decision,
      'flag',
      `compliance.enforcementFlagDecisions[${i}]`,
      reasons,
    )
    ...
    if (!['ON', 'OFF', 'DEFER'].includes(decision.decision)) {
      reasons.push(
        `compliance.enforcementFlagDecisions[${i}] (${flag ?? '?'}): decision = ${JSON.stringify(decision.decision)} — ON/OFF/DEFER kell`,
      )
    }
  ...
  for (const flag of codeFlags) {
    if (!decidedFlags.has(flag)) {
      reasons.push(
        `compliance: a kódban létező "${flag}" flagről nincs döntés (enforcementFlagDecisions)`,
      )
    }
  }
  for (const flag of decidedFlags.keys()) {
    if (!codeFlags.includes(flag)) {
      reasons.push(
        `compliance: a(z) "${flag}" flag NEM létezik a backend kódban — hallucinált flag-döntés tilos`,
      )
    }
  }
  return { reasons }
...
function checkDrDrill() {
  ...
  if (started && completed) {
    ...
    if (minutes <= 0) {
      ...
      reasons.push(
        `dr-drill: mért helyreállítási idő ${Math.round(minutes)} perc > ${MAX_DR_RESTORE_MINUTES} perc cél (roadmap DoD: helyreállítás 1 órán belül)`,
      )
    }
  }
  ...
  if (!checks || checks.length < 2) {
    reasons.push(
      'dr-drill.postRestoreChecks: legalább 2 helyreállítás-utáni ellenőrzés kell (pl. adat-integritás + app-bejelentkezés)',
    )
  } else {
    checks.forEach((check, i) => {
      ...
      if (check.result !== 'PASS') {
        reasons.push(
          `dr-drill.postRestoreChecks[${i}] (${check.name ?? '?'}): result = ${JSON.stringify(check.result)} — PASS kell`,
        )
      }
    })
  }
...
function checkMonitoring() {
  ...
  const end = requireDate(data, 'windowEnd', 'monitoring', reasons, {
    maxAgeDays: MAX_EVIDENCE_AGE_DAYS,
  })
  if (start && end) {
    ...
    if (hours < MIN_MONITORING_HOURS) {
      reasons.push(
        `monitoring: a mért ablak ${Math.floor(hours)} óra < ${MIN_MONITORING_HOURS} óra (roadmap DoD: 7 nap folyamatos monitoring)`,
      )
    }
  }
  ...
  if (typeof data.uptimePercent !== 'number' || data.uptimePercent < MIN_UPTIME_PERCENT) {
    reasons.push(
      `monitoring.uptimePercent = ${JSON.stringify(data.uptimePercent)} — szám kell, ≥ ${MIN_UPTIME_PERCENT}`,
    )
  }
  ...
  if (!incidents) {
    ...
    incidents.forEach((incident, i) => {
      ...
      if (incident.resolved !== true) {
        reasons.push(
          `monitoring.incidents[${i}]: resolved = ${JSON.stringify(incident.resolved)} — megoldatlan incidenssel nincs product-ready`,
        )
      }
    })
  }
...
function checkInstaller() {
  ...
  if (!signature || typeof signature !== 'object') {
    reasons.push(
      'installer.signature: hiányzó objektum (subject + issuer + thumbprint + timestamped)',
    )
  } else {
    ...
    if (signature.timestamped !== true) {
      reasons.push(
        'installer.signature.timestamped: true kell (időbélyeg nélküli aláírás lejár a tanúsítvánnyal)',
      )
    }
  }
  ...
  if (!vm || typeof vm !== 'object') {
    ...
    if (vm.cleanSnapshot !== true) {
      reasons.push(
        'installer.cleanVm.cleanSnapshot: true kell — a teszt csak tiszta VM-en érvényes',
      )
    }
    for (const step of ['install', 'launch', 'uninstall']) {
      ...
      if (result !== 'PASS') {
        reasons.push(
          `installer.cleanVm.steps.${step}.result = ${JSON.stringify(result)} — PASS kell`,
        )
      }
    }
  }
...
function checkFinalEvidence() {
  ...
  for (const id of requiredConfirmations) {
    if (data.confirmations?.[id] !== true) {
      reasons.push(
        `evidence.confirmations["${id}"]: true kell — minden rész-evidence tételes megerősítése kötelező`,
      )
    }
  }
  ...
  if (!signers || signers.length === 0) {
    reasons.push(
      'evidence.signedOffBy: legalább egy névvel/szereppel/dátummal aláírt sign-off kell',
    )
  } else {
    signers.forEach((signer, i) => {
      requireString(signer, 'name', `evidence.signedOffBy[${i}]`, reasons)
      requireString(signer, 'role', `evidence.signedOffBy[${i}]`, reasons)
      requireDate(signer, 'date', `evidence.signedOffBy[${i}]`, reasons, {
        maxAgeDays: MAX_EVIDENCE_AGE_DAYS,
      })
    })
  }
...
function checkInternal() {
  ...
  const sync = spawnSync(
    process.execPath,
    [path.join(repoRoot, 'scripts', 'check-version-sync.mjs')],
    {
      cwd: repoRoot,
      encoding: 'utf8',
    },
  ...
  if (sync.status !== 0) {
    reasons.push(
      `verzió-szinkron sértés: ${(sync.stderr || sync.stdout || '').trim().split('\n')[0]}`,
    )
  }
  ...
  if (!fs.existsSync(gatePath)) {
    reasons.push(
      'hiányzó security gate riport: security-reports/latest/gate-status.json (futtasd: scripts/security/run-security-gate.ps1)',
    )
  } else {
    try {
      ...
      if (gate.status !== 'PASSED') {
        reasons.push(
          `security gate státusz: ${JSON.stringify(gate.status)} — PASSED kell (riport: ${gate.timestamp ?? '?'})`,
        )
      }
      const age = Date.now() - new Date(gate.timestamp).getTime()
      if (!gate.timestamp || Number.isNaN(age) || age > 30 * 24 * 3600 * 1000) {
        reasons.push(
          `security gate riport elavult (${gate.timestamp ?? 'nincs timestamp'}) — 30 napnál frissebb kell`,
        )
      }
    } catch (err) {
...
const CHECKS = [
  ...
  {
    id: 'compliance',
    title: 'Compliance döntés + DB export + flag-döntések',
    run: checkCompliance,
  },
  ...
  {
    id: 'internal',
    title: 'Repo-oldali gépi DoD (manuálok, runbookok, verzió-sync, security gate)',
    run: checkInternal,
  },
]
...
function printResult(result, { verbose = true } = {}) {
  const mark =
    result.status === 'PASS' ? 'PASS   ' : result.status === 'MISSING' ? 'MISSING' : 'FAIL   '
...
function finalGate({ exitOnFail }) {
  console.log(
    `[product-ready] Verifier — repo verzió: ${rootVersion()} — evidence: docs/release-evidence/`,
  )
  ...
  if (failed.length === 0) {
    console.log(
      '[product-ready] VERDIKT: PRODUCT READY — mind a 8 kapu igazolt evidence-szel zöld.',
    )
    return 0
  }
  ...
  for (const result of failed) {
    console.log(
      `  - ${result.id}: ${result.status === 'MISSING' ? 'evidence fájl hiányzik' : `${result.reasons.length} hiba`}`,
    )
  }
...
if (command === 'status') {
  ...
  if (!check) {
    console.error(
      `[product-ready] ismeretlen check: "${command}". Választható: status, final-gate, ${CHECKS.map((c) => c.id).join(', ')}`,
    )
    process.exit(2)
  }
