...
try {
  ...
  const EXCLUDE =
    /(package-lock\.json|yarn\.lock|pnpm-lock\.yaml|\.snap$|dist\/|build\/|coverage\/|\.min\.(js|css)$)/
  let add = 0,
    del = 0,
    fileCount = 0
  ...
  console.log(
    `[pr-size] ${base}...HEAD: ${fileCount} fájl, +${add}/-${del} = ${total} sor (limit: ${limit}) → ${status}`,
  )
  if (total > limit) {
    console.log(
      '[pr-size] Javaslat: bontsd a változást kisebb, önállóan reviewolható szeletekre (TECH-053).',
    )
