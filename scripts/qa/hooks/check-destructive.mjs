...
try {
  ...
  if (
    /\b(?:remove-item|ri|del|erase)\b[^|;]*["']?[a-z]:[\\\/]?["']?\s*(?:-recurse|-force|\s|$)/.test(
      cmd,
    ) &&
  ...
  if (
    /\b(?:rd|rmdir|rm|remove-item|del)\b[^|;]*[a-z]:[\\\/](?:windows|users)["'\\\/ ]*(?:\s|$)/.test(
      cmd,
    ) &&
  ...
  if (/\bformat(?:-volume)?\b\s+(?:-driveletter\s+)?["']?[a-z]:?\b/.test(cmd))
    deny('meghajtó formázása')
  ...
  if (/\bdrop\s+(?:table|database|schema)\b/.test(cmd))
    deny('DROP TABLE/DATABASE/SCHEMA — void/reversal elv sérülne')
  if (/\bprisma\s+migrate\s+reset\b/.test(cmd))
    deny('prisma migrate reset — teljes DB-újrahúzás; emberi döntést igényel')

  // git push --force védett ágra (a --force-with-lease nem esik bele)
  const hasForce = /\bgit\b[^|;&]*\bpush\b[^|;&]*(?:--force(?!-with-lease)|\s-f\b)/.test(cmd)
  if (hasForce && /\b(main|master)\b/.test(cmd)) deny('git push --force main/master ágra')

  // ---- WARN: határesetek (engedélyezett, de jelez) --------------------------

  if (hasForce)
    warn(
      'git push --force (nem védett ágra) — győződj meg róla, hogy senki más nem dolgozik az ágon.',
    )
