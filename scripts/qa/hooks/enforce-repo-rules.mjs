...
try {
  ...
  for (const r of loadRepoRules()) {
    try {
      ...
      /* hibás egyedi szabály — kihagy, fail-open */
    }
  }
  ...
  if (isCmd) {
    ...
    if (
      /\bgit\b[^|;&]*\b(?:commit|push|merge)\b[^|;&]*--no-verify\b/.test(c) ||
  ...
  if (isEdit && content) {
    ...
    const secretAssign =
      /(password|passwd|secret|api[_-]?key|access[_-]?key|client[_-]?secret|auth[_-]?token|token)\s*[:=]\s*["'`]([^"'`$\n]{8,})["'`]/i
    const m = content.match(secretAssign)
    if (
      m &&
      !/(\$\{|process\.env|os\.environ|getenv|<[^>]+>|example|placeholder|xxx+|\*{3,}|changeme|your[_-]?|dummy|sample)/i.test(
        m[2],
      )
    )
      warn(
        `lehetséges hard-coded secret (${m[1]}=...) — titok kódba/commitba TILOS; használj env/secret-store-t, placeholderrel.`,
      )
    ...
    if (/\beval\s*\(/.test(content) || /\bnew\s+Function\s*\(/.test(content))
      warn(
        'eval / new Function használat — kódinjekció-kockázat; kerüld vagy validáld szigorúan a bemenetet.',
      )
    ...
    if (/\.(test|spec)\.[cm]?[jt]sx?$/i.test(fp) || /_test\.py$|test_.*\.py$/i.test(fp)) {
      if (
        /\b(?:it|test|describe)\.(?:skip|only)\b|@pytest\.mark\.skip|xit\(|xdescribe\(/.test(
          content,
        )
      )
        warn(
          'teszt skip/only — tesztet a bukás elkerülésére kihagyni TILOS; az implementációt javítsd.',
        )
    }
  }
